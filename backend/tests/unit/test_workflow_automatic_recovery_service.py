...
边界：禁止启动 Runtime；通过替换依赖验证 eligible / rejected / recovered。
关键依赖：WorkflowExecutionAutomaticRecoveryService、WorkflowRecoveryEventLogger。
"""
...
@pytest.mark.asyncio
async def test_recover_emits_recovered_attempt_event(monkeypatch) -> None:
    event_logger = _EventLogger()
    service = WorkflowExecutionAutomaticRecoveryService(
        db=object(), policy=WorkflowExecutionRecoveryPolicy(max_attempts=3, cooldown_seconds=0), event_logger=event_logger,
    )
    ...
    class _ExecutionService:
        ...
        async def resume_from_latest_checkpoint(self, _execution, _actor):
            return SimpleNamespace(id=uuid4())

    import app.services.workflow.execution as execution_module
    monkeypatch.setattr(execution_module, "WorkflowExecutionService", _ExecutionService)

    result = await service.recover(execution, now=datetime(2026, 8, 26, 12, 0))

    assert result.outcome == "recovered"
    assert result.resume_execution_id is not None
    assert event_logger.events[0].event_name == RECOVERY_ATTEMPT
