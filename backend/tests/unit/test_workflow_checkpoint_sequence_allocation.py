...
async def test_append_next_locks_execution_before_reading_sequence() -> None:
    ...
    execution = MagicMock(); execution.status = "running"; execution.worker_owner = None; execution.worker_attempt = 0; execution.worker_lease_expires_at = None
    first_result = MagicMock(); first_result.scalar_one_or_none.return_value = execution
    second_result = MagicMock(); second_result.scalar_one.return_value = 7
    ...
    checkpoint = await service.append_next_in_transaction(
        execution_id=uuid4(), execution_status="running", state_data={"frontier": ["node-b"]},
        checkpoint_reason="node.completed", node_id="node-a", node_attempt=1, node_status="succeeded", output_data={"value": 1},
    )
...
async def test_append_next_rejects_unknown_execution() -> None:
    db = MagicMock(); db.execute = AsyncMock(); db.add = MagicMock(); db.flush = AsyncMock()
    result = MagicMock(); result.scalar_one_or_none.return_value = None; db.execute.return_value = result
    service = WorkflowExecutionCheckpointService(db)
    with pytest.raises(HTTPException, match="Execution 不存在"):
        await service.append_next_in_transaction(
            execution_id=uuid4(), execution_status="running", state_data={}, checkpoint_reason="node.completed",
            node_id="node-a", node_attempt=1, node_status="succeeded",
        )
