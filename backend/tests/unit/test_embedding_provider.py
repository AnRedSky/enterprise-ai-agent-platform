"""OpenAI-compatible Embedding Provider 的协议与错误行为测试。"""
...
async def test_openai_compatible_embedding_provider_orders_vectors_by_index() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={"data": [{"index": 1, "embedding": [2, 3]}, {"index": 0, "embedding": [0, 1]}]})

    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    provider = OpenAICompatibleEmbeddingProvider(base_url="http://embedding.local/v1", api_key="test-key", model="text-embedding-test", expected_dimension=2, client=client)
    try:
...
async def test_openai_compatible_embedding_provider_can_request_output_dimensions() -> None:
    ...
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    provider = OpenAICompatibleEmbeddingProvider(base_url="http://localhost:11434/v1", api_key="ollama", model="qwen3-embedding:4b", dimensions=1536, expected_dimension=2, client=client)
    try:
...
async def test_openai_compatible_embedding_provider_rejects_dimension_mismatch() -> None:
    client = httpx.AsyncClient(transport=httpx.MockTransport(lambda _: httpx.Response(200, json={"data": [{"index": 0, "embedding": [0.1, 0.2]}]})))
    provider = OpenAICompatibleEmbeddingProvider(base_url="http://embedding.local/v1", api_key="test-key", model="text-embedding-test", expected_dimension=3, client=client)
    try:
...
async def test_openai_compatible_embedding_provider_rejects_inconsistent_batch_dimensions() -> None:
    client = httpx.AsyncClient(transport=httpx.MockTransport(lambda _: httpx.Response(200, json={"data": [{"index": 0, "embedding": [0.1, 0.2]}, {"index": 1, "embedding": [0.1, 0.2, 0.3]}]})))
    provider = OpenAICompatibleEmbeddingProvider(base_url="http://embedding.local/v1", api_key="test-key", model="text-embedding-test", client=client)
    try:
...
async def test_openai_compatible_embedding_provider_rejects_http_errors() -> None:
    client = httpx.AsyncClient(transport=httpx.MockTransport(lambda _: httpx.Response(503, json={"error": "unavailable"})))
    provider = OpenAICompatibleEmbeddingProvider(base_url="http://embedding.local/v1", api_key="test-key", model="text-embedding-test", client=client)
    try:
