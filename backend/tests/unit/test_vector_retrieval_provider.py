"""Vector Provider 的排序、作用域与配置校验测试。"""
...
async def test_in_memory_vector_provider_ranks_by_cosine_similarity() -> None:
    ...
    await provider.upsert([VectorRecord("chunk-a", (1.0, 0.0), {"source": "a"}), VectorRecord("chunk-b", (0.0, 1.0), {"source": "b"}), VectorRecord("chunk-c", (0.8, 0.2), {"source": "c"})])
...
async def test_in_memory_vector_provider_applies_min_score_and_knowledge_scope() -> None:
    provider = InMemoryVectorRetrievalProvider()
    await provider.upsert([VectorRecord("chunk-b", (1.0, 0.0), {"knowledge_base_id": "kb-2"}), VectorRecord("chunk-a", (1.0, 0.0), {"knowledge_base_id": "kb-1"}), VectorRecord("chunk-c", (0.0, 1.0), {"knowledge_base_id": "kb-1"})])
