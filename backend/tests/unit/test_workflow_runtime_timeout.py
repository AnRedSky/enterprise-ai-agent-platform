...
async def test_run_exhausts_retry_budget_before_scheduling_retry(monkeypatch):
    ...
    version = SimpleNamespace(
        definition={
            "config": {"timeout_ms": 1000, "retry_budget": {"max_retries": 0}},
            "nodes": [{
                "id": "transient",
                "type": "input",
                "config": {
                    "timeout_ms": 1000,
                    "retry": {"max_attempts": 3, "backoff_ms": 1, "jitter_ms": 0},
                },
            }],
        }
    )
...
async def test_run_exhausts_retry_when_backoff_crosses_workflow_deadline(monkeypatch):
    ...
    version = SimpleNamespace(
        definition={
            "config": {"timeout_ms": 10, "retry_budget": {"max_retries": 3}},
            "nodes": [{
                "id": "transient",
                "type": "input",
                "config": {
                    "timeout_ms": 1000,
                    "retry": {"max_attempts": 3, "backoff_ms": 100, "max_backoff_ms": 100, "jitter_ms": 0},
                },
            }],
        }
    )
    ...
    assert exc.value.status_code == 504
    assert execution.status == "failed"
    trace = service.governance.trace.await_args_list[-1]
