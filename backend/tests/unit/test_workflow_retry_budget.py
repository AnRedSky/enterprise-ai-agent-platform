...
async def test_retry_budget_exhaustion_fails_execution_and_records_governance(monkeypatch):
    ...
    execution = SimpleNamespace(
        id=uuid4(), tenant_id=uuid4(), created_by=uuid4(), status="pending",
        input_data={"input": "retry"}, started_at=None, ended_at=None,
        current_node_id=None, output_data=None, error_code=None, error_message=None,
    )
    version = SimpleNamespace(definition={
        "config": {"timeout_ms": 1000, "retry_budget": {"max_retries": 1}},
        "nodes": [{
            "id": "unstable", "type": "input",
            "config": {
                "timeout_ms": 1000,
                "retry": {"max_attempts": 5, "backoff_ms": 0, "max_backoff_ms": 0,
    ...
    async def transition_node(execution, node_id, target_status, **kwargs):
        nonlocal node_calls
        node_calls += 1
        attempt = 1 if node_calls <= 2 else 2
    ...
    assert execution.error_code == "HTTP_503"
    assert any(
        call.args[2] == "node.retry.exhausted" and (call.kwargs.get("data") or {}).get("reason") == "retry_budget"
        for call in service.governance.trace.await_args_list
    )
...
async def test_retry_deadline_exhaustion_marks_workflow_timeout(monkeypatch):
    ...
    execution = SimpleNamespace(
        id=uuid4(), tenant_id=uuid4(), created_by=uuid4(), status="pending",
        input_data={"input": "deadline"}, started_at=None, ended_at=None,
        current_node_id=None, output_data=None, error_code=None, error_message=None,
    )
    version = SimpleNamespace(definition={
        "config": {"timeout_ms": 10, "retry_budget": {"max_retries": 5}},
        "nodes": [{
            "id": "unstable", "type": "input",
            "config": {
                "timeout_ms": 1000,
                "retry": {"max_attempts": 3, "backoff_ms": 100, "max_backoff_ms": 100,
                           "jitter_ms": 0, "retryable_error_codes": ["HTTP_503"]},
            },
        }],
    })
    ...
    service.transition = AsyncMock(side_effect=transition)
    service.transition_node = AsyncMock(return_value=SimpleNamespace(attempt=1))
    ...
    assert execution.status == "failed"
    assert execution.error_code == "WORKFLOW_TIMEOUT"
    assert any(
        call.args[2] == "node.retry.exhausted" and (call.kwargs.get("data") or {}).get("reason") == "workflow_deadline"
        for call in service.governance.trace.await_args_list
    )
