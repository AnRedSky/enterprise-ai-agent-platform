...
def _ready() -> WorkflowDagJoinReadiness:
    return WorkflowDagJoinReadiness(
        node_id="join", predecessor_node_ids=("a", "b"), ready=True, state_data={"a": 1, "b": 2}
    )
...
async def test_join_executor_persists_after_successful_execution() -> None:
    ...
    result = await WorkflowDagJoinExecutor.execute(
        _ready(), node={"id": "join"}, executor=executor, persister=persister
    )
...
async def test_join_executor_reuses_completed_fact_without_execution() -> None:
    ...
    result = await WorkflowDagJoinExecutor.execute(
        _ready(), node={"id": "join"}, executor=executor, already_completed=True
    )
