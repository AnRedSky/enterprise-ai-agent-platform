...
def test_retry_policy_rejects_unbounded_attempts_and_invalid_backoff():
    with pytest.raises(HTTPException): WorkflowRuntime.resolve_retry_policy({"retry": {"max_attempts": 6}})
    with pytest.raises(HTTPException): WorkflowRuntime.resolve_retry_policy({"retry": {"backoff_ms": -1}})
    with pytest.raises(HTTPException): WorkflowRuntime.resolve_retry_policy({"retry": {"max_backoff_ms": 100, "backoff_ms": 200}})


def test_retry_error_classification_only_marks_transient_errors_retryable_by_policy():
    policy = WorkflowRuntime.resolve_retry_policy({"retry": {"max_attempts": 3, "retryable_error_codes": ["HTTP_503", "NODE_TIMEOUT"]}})
    assert "HTTP_503" in policy["retryable_error_codes"]; assert "HTTP_422" not in policy["retryable_error_codes"]
...
def test_retry_delay_is_bounded_and_exponential():
    policy = WorkflowRuntime.resolve_retry_policy({"retry": {"max_attempts": 4, "backoff_ms": 100, "max_backoff_ms": 500, "jitter_ms": 50}})
...
async def test_failed_node_can_resume_as_next_attempt():
    ...
    async def trace(*args, **kwargs): return None
    service.governance.trace = trace
    class FakeResult:
        def scalar_one_or_none(self): return node
    class FakeDB:
        def __init__(self): self.result = FakeResult()
    ...
    node = SimpleNamespace(status="failed", attempt=1, started_at=None, ended_at=None, input_data=None, output_data=None, error_code="HTTP_503", error_message="temporary")
    execution = SimpleNamespace(id=uuid4(), status="running", tenant_id=uuid4(), created_by=uuid4(), current_node_id=None)
    service.db = FakeDB()
    result = await service.transition_node(execution, "agent", "running", input_data={"x": 1})
    assert result.attempt == 2; assert result.status == "running"; assert result.ended_at is None; assert execution.current_node_id == "agent"
