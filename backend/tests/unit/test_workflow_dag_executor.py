...
async def test_multi_frontier_persists_each_branch_before_join() -> None:
    ...
    result = await WorkflowDagMultiFrontierExecutor.execute(
        _plan(), branch_state_data={"branch-a": {}, "branch-b": {}}, executor=executor, checkpoint_writer=checkpoint_writer,
    )
    assert checkpoints == [("branch-a", {"branch-a": "completed"}), ("branch-b", {"branch-b": "completed"})]
...
async def test_multi_frontier_rejects_missing_branch_state() -> None:
    ...
    with pytest.raises(ValueError, match="缺少 Branch state"):
        await WorkflowDagMultiFrontierExecutor.execute(_plan(), branch_state_data={"branch-a": {"a": 1}}, executor=executor)


@pytest.mark.asyncio
async def test_multi_frontier_without_checkpoint_never_declares_join_ready_even_with_overlapping_keys() -> None:
    async def executor(node, state):
        state["shared"] = node["id"]
        return state
    result = await WorkflowDagMultiFrontierExecutor.execute(
        _plan(), branch_state_data={"branch-a": {}, "branch-b": {}}, executor=executor,
    )
    assert result.join_ready is False
    assert result.merged_state_data is None
...
async def test_branch_failure_does_not_reach_later_branch() -> None:
    ...
    with pytest.raises(RuntimeError, match="branch failed"):
        await WorkflowDagMultiFrontierExecutor.execute(_plan(), branch_state_data={"branch-a": {}, "branch-b": {}}, executor=executor)
    assert executed == ["branch-a"]
