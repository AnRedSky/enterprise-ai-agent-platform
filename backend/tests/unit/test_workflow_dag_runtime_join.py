...
def join_definition():
    return {
        "nodes": [
            {"id": "root", "type": "input"}, {"id": "left", "type": "input"},
            {"id": "right", "type": "input"}, {"id": "join", "type": "join"},
            {"id": "output", "type": "output"},
        ],
        "edges": [
            {"source": "root", "target": "left"}, {"source": "root", "target": "right"},
...
async def test_join_node_is_pure_state_aggregation():
    ...
    result = await runtime.execute_node({"id": "join", "type": "join", "config": {}}, state, uuid4(), False, uuid4())
    assert result == state
...
async def test_resume_context_uses_persisted_predecessor_outputs_for_join(join_definition):
    ...
    result = SimpleNamespace(scalars=lambda: SimpleNamespace(all=lambda: [root, left, right]))
    db.execute = AsyncMock(return_value=result)
    tenant_id = uuid4()
    execution = SimpleNamespace(id=uuid4(), tenant_id=tenant_id, resume_of_execution_id=uuid4(), workflow_version_id=uuid4())
...
async def test_join_runtime_does_not_use_stale_resume_input(join_definition):
    ...
    execution = SimpleNamespace(id=uuid4(), tenant_id=uuid4(), resume_of_execution_id=uuid4(), workflow_version_id=uuid4())
    runtime = WorkflowRuntime(db)
...
@pytest.mark.asyncio
async def test_runtime_continues_persisted_recovery_trace_to_base_runtime(monkeypatch):
    ...
    base_execute = AsyncMock(return_value={"ok": True})
    monkeypatch.setattr(BaseWorkflowRuntime, "execute", base_execute)

    runtime = WorkflowRuntime(db)
    result = await runtime.execute(execution, version, actor_id)
    assert result == {"ok": True}
    base_execute.assert_awaited_once_with(execution, version, actor_id)
