...
async def test_automatic_recovery_uses_one_trace_for_attempt_and_finish() -> None:
    ...
    async def resume_with_outcome(_execution, _actor_id):
        return SimpleNamespace(
            execution=SimpleNamespace(id=resume_id),
            outcome="created",
        )
    ...
    assert trace_events[0].trace_id is not None
    assert trace_events[1].trace_id == trace_events[0].trace_id
    assert trace_events[2].trace_id == trace_events[0].trace_id
    assert trace_events[1].resume_execution_id == resume_id
    assert trace_events[2].resume_execution_id == resume_id
    assert trace_events[1].phase == "automatic_recovery"
    assert trace_events[2].phase == "automatic_recovery"
...
async def test_automatic_recovery_rejection_also_closes_trace() -> None:
    ...
    async def evaluate(_execution, *, now=None):
        return SimpleNamespace(
            decision=WorkflowExecutionRecoveryDecision(
                eligible=False,
                reason_code="checkpoint_not_eligible",
                attempt_count=1,
                max_attempts=3,
            )
        )
