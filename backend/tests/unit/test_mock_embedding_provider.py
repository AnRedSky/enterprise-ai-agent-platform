"""Mock Embedding Provider 的离线确定性行为测试。"""

import pytest

from app.infrastructure.providers.embedding import EmbeddingProviderError
from app.infrastructure.providers.mock_embedding import MockEmbeddingProvider
...
async def test_mock_embedding_provider_preserves_shared_token_similarity() -> None:
    ...
    vectors = await provider.embed(["FastAPI Agent Runtime", "FastAPI Agent Runtime execution", "expense reimbursement policy"])
...
async def test_mock_embedding_provider_matches_chinese_query_inside_longer_chunk() -> None:
    ...
    vectors = await provider.embed(["报销规则", "报销规则规定员工报销的申请条件、金额限制、票据要求以及审批流程。", "PostgreSQL 知识库用于保存企业知识文档、文档版本、Chunk 以及检索索引信息。"])
