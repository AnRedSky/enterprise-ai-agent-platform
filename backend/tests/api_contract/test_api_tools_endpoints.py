...
def test_tool_endpoints_require_bearer_authentication():
    tool="00000000-0000-0000-0000-000000000001"; agent="00000000-0000-0000-0000-000000000002"
    ...
    assert client.post(f"/api/v1/tools/{tool}/execute",json={"agent_id":agent,"arguments":{}}).status_code==401

@pytest.mark.asyncio
async def test_admin_tool_list_includes_disabled_tools():
    disabled=SimpleNamespace(id="disabled",enabled=False); enabled=SimpleNamespace(id="enabled",enabled=True)
    db=AsyncMock(); result=MagicMock(); result.scalars.return_value.all.return_value=[disabled,enabled]; db.execute.return_value=result
...
async def test_user_tool_list_filters_disabled_tools():
    enabled=SimpleNamespace(id="enabled",enabled=True); db=AsyncMock(); result=MagicMock(); result.scalars.return_value.all.return_value=[enabled]; db.execute.return_value=result
