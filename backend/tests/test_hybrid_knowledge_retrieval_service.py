...
async def test_hybrid_retrieval_combines_real_service_outputs(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.hybrid_knowledge_retrieval_service.KnowledgeRetrievalService.retrieve",
        fake_lexical,
    )
    monkeypatch.setattr(
        "app.services.hybrid_knowledge_retrieval_service.VectorKnowledgeRetrievalService.retrieve",
        fake_vector,
    )

    results = await HybridKnowledgeRetrievalService(None).retrieve(
        query="q",
        top_k=3,
        owner_id=None,
        lexical_weight=0.4,
...
async def test_hybrid_applies_min_score_after_fusion(monkeypatch):
    ...
    results = await HybridKnowledgeRetrievalService(None).retrieve(
        query="q", top_k=5, owner_id=None, min_score=0.45
    )
