...
@pytest.mark.asyncio
async def test_owner_cannot_see_other_execution(db):
...
async def test_admin_can_see_all_and_filters_apply(db):
    ...
    service = RuntimeQueryService(db)
    _, _, total, rows = await service.executions(admin, True, page=1, page_size=100, status="success", agent_id=agent_a, trace_id="trace-a")
    assert total == 1 and rows[0].id == execution_a

@pytest.mark.asyncio
async def test_owner_audit_scope_isolated(db):
