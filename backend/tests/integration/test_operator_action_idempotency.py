"""Operator Action PostgreSQL 集成测试：验证幂等键并发 claim 与租户隔离。

职责：验证 OperatorActionIdempotency 的数据库唯一约束与原子 claim 行为。
...
async def test_operator_action_idempotency_concurrent_claim_has_single_winner_and_tenant_isolation() -> None:
    """验证同租户并发 claim 只有一个新建者，同时相同 key 在不同租户可以独立使用。"""
    tenant_id = uuid4()
    ...
    try:
        ...
        async with SessionLocal() as cleanup_session:
            await cleanup_session.execute(
                delete(OperatorActionIdempotency).where(
                    OperatorActionIdempotency.tenant_id.in_([tenant_id, other_tenant_id])
                )
            )
            await cleanup_session.execute(delete(User).where(User.id.in_([user_id, other_user_id])))
            await cleanup_session.execute(delete(Tenant).where(Tenant.id.in_([tenant_id, other_tenant_id])))
