...
职责：在既有真实 Resume acceptance 基础上验证三节点线性 DAG 的完整 Definition 不被 Worker 裁剪，恢复后从单一 frontier 连续执行到尾节点。
        ...
        try:
            with _client() as client:
                provider = client.post("/model-providers", json={
                    "organization_id": __import__("os").environ["ORGANIZATION_ID"],
                ...
                execution = client.post(f"/workflows/{workflow_id}/executions", json={"input_data": {"source": "full-dag-resume"}})
            ...
            async with SessionLocal() as db:
                source_nodes = (await db.execute(
                    select(WorkflowNodeExecution).where(WorkflowNodeExecution.execution_id == source.id)
                    .order_by(WorkflowNodeExecution.created_at.asc(), WorkflowNodeExecution.id.asc())
                )).scalars().all()
                checkpoints = (await db.execute(
                    select(WorkflowExecutionCheckpoint).where(WorkflowExecutionCheckpoint.execution_id == source.id)
            ...
            async with SessionLocal() as db:
                source = (await db.execute(select(WorkflowExecution).where(WorkflowExecution.id == source_id))).scalar_one()
            ...
            async with SessionLocal() as db:
                resume_nodes = (await db.execute(
                    select(WorkflowNodeExecution).where(WorkflowNodeExecution.execution_id == resume.id)
                    .order_by(WorkflowNodeExecution.created_at.asc(), WorkflowNodeExecution.id.asc())
                )).scalars().all()
                resume_checkpoints = (await db.execute(
                    select(WorkflowExecutionCheckpoint).where(WorkflowExecutionCheckpoint.execution_id == resume.id)
                    .order_by(WorkflowExecutionCheckpoint.sequence.asc())
                )).scalars().all()
