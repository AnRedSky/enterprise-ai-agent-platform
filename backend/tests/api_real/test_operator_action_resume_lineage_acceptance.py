...
async def test_resume_operator_action_replay_reuses_result_and_audit() -> None:
    ...
    try:
        async with SessionLocal() as db:
            ...
            user = User(id=user_id, username=f"phase-210-resume-user-{suffix}", password_hash="test", tenant_id=tenant_id)
            workflow = Workflow(id=workflow_id, tenant_id=tenant_id, owner_id=user_id, name=f"phase-210-resume-workflow-{suffix}", status="draft", published_version_id=None)
            version = WorkflowVersion(id=version_id, workflow_id=workflow_id, version="1.0.0", created_by=user_id, definition={"nodes": [{"id": "input", "type": "input", "config": {}}], "config": {}}, status="published")
            db.add_all([tenant, user, workflow, version])
            await db.flush()
            workflow.status = "published"
            workflow.published_version_id = version_id
            db.add(WorkflowExecution(id=execution_id, tenant_id=tenant_id, workflow_id=workflow_id, workflow_version_id=version_id, created_by=user_id, status="failed", input_data={"phase": "2.10-II", "scenario": "resume_idempotency"}, error_code="NODE_TIMEOUT"))
            await db.flush()
        ...
        async with SessionLocal() as db:
            first = await OperatorActionGovernanceService(db).execute_execution(execution_id=execution_id, tenant_id=tenant_id, actor_id=user_id, is_admin=True, action="resume", confirm=True)
            first_id = first.id
        ...
        async with SessionLocal() as db:
            replay = await OperatorActionGovernanceService(db).execute_execution(execution_id=execution_id, tenant_id=tenant_id, actor_id=user_id, is_admin=True, action="resume", confirm=True)
            assert replay.id == first_id
            assert replay.resume_of_execution_id == execution_id
            assert replay.resume_checkpoint_sequence == 1

        async with SessionLocal() as db:
            db.add(WorkflowExecutionCheckpoint(execution_id=execution_id, sequence=2, node_id="input", node_attempt=2, execution_status="running", node_status="completed", state_data={"resume": "state", "sequence": 2}, input_data={"phase": "2.10-II", "checkpoint": 2}, output_data={"node": "completed", "attempt": 2}, checkpoint_reason="node.completed", worker_owner=None, error_code=None))
            await db.commit()

        async with SessionLocal() as db:
            second = await OperatorActionGovernanceService(db).execute_execution(execution_id=execution_id, tenant_id=tenant_id, actor_id=user_id, is_admin=True, action="resume", confirm=True)
        ...
        async with SessionLocal() as db:
            action_rows = list((await db.execute(select(OperatorActionIdempotency).where(OperatorActionIdempotency.tenant_id == tenant_id, OperatorActionIdempotency.resource_type == "workflow_execution", OperatorActionIdempotency.resource_id == execution_id, OperatorActionIdempotency.action == "resume").order_by(OperatorActionIdempotency.created_at.asc()))).scalars().all())
            assert len(action_rows) == 2
            assert [row.idempotency_key for row in action_rows] == [f"internal:resume:{execution_id}:1", f"internal:resume:{execution_id}:2"]
            ...
            audits = list((await db.execute(select(AuditLog).where(AuditLog.tenant_id == tenant_id, AuditLog.action == "operator.workflow_execution.resume", AuditLog.operator_action_id.in_([row.id for row in action_rows])).order_by(AuditLog.created_at.asc()))).scalars().all())
            assert len(audits) == 2
            ...
            traces = list((await db.execute(select(WorkflowTraceEvent).where(WorkflowTraceEvent.tenant_id == tenant_id, WorkflowTraceEvent.execution_id.in_([first_id, second.id])))).scalars().all())
            ...
            for action, expected_execution_id in zip(action_rows, [first_id, second.id], strict=True):
                correlation = await RuntimeAuditTraceCorrelationService(db).by_operator_action(tenant_id, action.id)
