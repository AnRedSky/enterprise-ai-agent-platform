...
职责：复现 B6 多 Worker Delegation Claim/Execute 边界，并在未收敛时输出真实 PostgreSQL durable facts。
...
async def test_b6_multi_worker_diagnostic_dump_on_nonterminal_runtime() -> None:
    """复现 B6 多 Worker 消费，并在有界 drain 后输出 durable state，不隐藏真实失败。"""
    suffix = uuid.uuid4().hex[:10]
    ids: list[uuid.UUID] = []
    with _client() as client:
        for index in range(4):
            delegation_id, _, _, _, _ = await _create_delegation(client, f"b6-diagnostic-{suffix}-{index}")
