...
async def test_b1_atomic_claim_allows_one_worker_and_persists_one_execution():
    ...
    async with SessionLocal() as db:
        identity = (await db.execute(select(AgentDelegation).where(AgentDelegation.id == uuid.UUID(delegation_id)))).scalar_one()
    ...
    async with SessionLocal() as db:
        ...
        execution_row = (await db.execute(select(WorkflowExecution).where(WorkflowExecution.id == persisted.worker_execution_id))).scalar_one()
        ...
        assert "不能再次 Claim" in str(second.value)
