...
职责：统一处理独立 Worker 与 HTTP `/run` 请求之间的合法 pending 抢占竞态。
...
def run_or_observe_execution(
    ...
    *,
    ...
    """通过真实 HTTP `/run` 触发 Execution，并兼容 Worker 已先行抢占的合法竞态。

    Args:
        client: 已完成认证的真实 HTTP 客户端。
        execution_id: 待执行的 Workflow Execution ID。
        expected_http_status: 未发生 Worker 抢占时 `/run` 的预期 HTTP 状态码。
    ...
    if response.status_code != 409 or response.json().get("detail") != _PENDING_RUN_CONFLICT:
        raise AssertionError(
            ...
            f"{expected_http_status} or legal Worker claim race, got {response.status_code}: {response.text}"
        )
