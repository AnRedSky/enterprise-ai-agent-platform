"""Workflow Governance 领域服务。

职责：持久化 Workflow Execution 的审计事实、Trace 事件及模型调用用量核算。
边界：不负责 Workflow 状态机、节点执行或模型 Provider 调用；只消费 Execution 产生的治理事件。
关键依赖：AuditLog、WorkflowTraceEvent、ModelProfile 与 UsageAccountingService。
"""
...
class WorkflowGovernanceService:
    """持久化 Workflow 治理事件并在模型调用事件上执行用量核算。"""
