...
class KnowledgeRetrievalService:
    """Provider-neutral retrieval with deterministic quality and safety controls.

    The implementation remains infrastructure-neutral for Phase 1.4-E. It uses
    lexical matching while exposing production-oriented controls that can later be
    mapped to vector/hybrid providers without changing the API contract.
    """
    ...
    def _tokens(text: str) -> set[str]:
        """Return deterministic ASCII words plus Chinese sequence/bigram tokens."""
        tokens: set[str] = set()
    ...
    def _score_details(cls, query: str, content: str) -> tuple[float, list[str]]:
        ...
        score = min(1.0, round(token_recall * 0.85 + phrase_bonus, 6))
    ...
    async def retrieve(
        ...
        # Authorization is deliberately part of the retrieval query, before any
        # candidate can enter the ranking/context pipeline.
        if not is_admin:
