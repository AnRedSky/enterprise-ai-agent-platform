...
class ToolObservabilityAdapter:
    ...
    async def start_tool_span(self, context):
        self.started_at[context.tool_id] = datetime.utcnow()
    ...
    async def fail_tool_span(self, context, error):
        await self._record(context, "failed", getattr(error, "code", "TOOL_EXECUTION_ERROR"), str(error))

    async def _record(self, context, status, error_code=None, error_message=None):
        started = self.started_at.pop(context.tool_id, datetime.utcnow())
