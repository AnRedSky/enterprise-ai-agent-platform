...
from app.services.knowledge.vector_indexing import KnowledgeVectorIndexingService
...
class KnowledgeIngestionService:
    ...
    async def _get_version(self, version_id: UUID, owner_id: UUID, is_admin: bool) -> KnowledgeDocumentVersion:
        stmt = (select(KnowledgeDocumentVersion).join(KnowledgeDocument, KnowledgeDocument.id == KnowledgeDocumentVersion.document_id).join(KnowledgeBase, KnowledgeBase.id == KnowledgeDocument.knowledge_base_id).where(KnowledgeDocumentVersion.id == version_id))
        if not is_admin:
    ...
    async def ingest(self, version_id: UUID, owner_id: UUID, is_admin: bool = False, *, max_chars: int = 1000, overlap_chars: int = 100) -> tuple[KnowledgeDocumentVersion, int]:
    ...
    async def list_chunks(self, version_id: UUID, owner_id: UUID, is_admin: bool = False) -> list[KnowledgeDocumentChunk]:
        ...
        stmt = select(KnowledgeDocumentChunk).where(KnowledgeDocumentChunk.document_version_id == version_id).order_by(KnowledgeDocumentChunk.chunk_index.asc())
