...
class HybridRetrievalService:
    ...
    def fuse(self, lexical: Sequence[HybridCandidate], vector: Sequence[HybridCandidate], top_k: int) -> list[HybridCandidate]:
        if top_k < 1:
        ...
        for candidate in lexical:
            ...
            lexical_scores[candidate.chunk_id] = max(lexical_scores.get(candidate.chunk_id, 0.0), candidate.score)
            candidates.setdefault(candidate.chunk_id, candidate)
        for candidate in vector:
            self._validate_candidate(candidate)
            vector_scores[candidate.chunk_id] = max(vector_scores.get(candidate.chunk_id, 0.0), candidate.score)
            candidates.setdefault(candidate.chunk_id, candidate)
        ...
        for chunk_id, candidate in candidates.items():
            ...
            if left is not None and right is not None:
                score = (self.config.lexical_weight * left + self.config.vector_weight * right) / total
            elif left is not None:
                score = left
            else:
                score = right
            sources = (["lexical"] if left is not None else []) + (["vector"] if right is not None else [])
            payload = dict(candidate.payload)
            payload["hybrid_score_breakdown"] = {"lexical_score": left, "vector_score": right, "lexical_weight": self.config.lexical_weight, "vector_weight": self.config.vector_weight, "fused_score": round(score, 6), "support": sources}
            fused.append(HybridCandidate(chunk_id, round(score, 6), "+".join(sources), payload))
        fused.sort(key=lambda item: (-(1 if item.source == "lexical+vector" else 0), -item.score, item.chunk_id))
        return fused[:top_k]
    @staticmethod
