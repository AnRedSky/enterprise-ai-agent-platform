...
async def complete_delegation(*, db: AsyncSession, tenant_id: UUID, delegation_id: UUID, worker_execution_id: UUID) -> AgentDelegation:
    """以当前 Worker generation 原子完成 Delegation。"""
    delegation, execution = await _lock_delegation(db, tenant_id=tenant_id, delegation_id=delegation_id, worker_execution_id=worker_execution_id)
    ...
    await db.flush()
    db.add(AuditLog(actor_id=execution.created_by, tenant_id=tenant_id, workflow_id=execution.workflow_id, workflow_version_id=execution.workflow_version_id,
                    workflow_execution_id=delegation.source_execution_id, action="workflow.delegation.completed", resource_type="agent_delegation",
                    resource_id=str(delegation.id), trace_id=delegation.trace_id, status="success",
                    metadata_json={"worker_execution_id": str(worker_execution_id)}))
    db.add(WorkflowTraceEvent(tenant_id=tenant_id, execution_id=delegation.source_execution_id, workflow_id=execution.workflow_id,
                              workflow_version_id=execution.workflow_version_id, event_type="agent.delegation.completed", status="completed",
                              trace_id=delegation.trace_id, actor_id=execution.created_by,
...
async def fail_delegation(*, db: AsyncSession, tenant_id: UUID, delegation_id: UUID, worker_execution_id: UUID,
                          error_code: str, error_message: str) -> AgentDelegation:
    ...
    delegation.ended_at = utcnow_naive()
    delegation.error_code = normalized_error_code
    delegation.error_message = normalized_error_message
    await db.flush()
    db.add(AuditLog(actor_id=execution.created_by, tenant_id=tenant_id, workflow_id=execution.workflow_id, workflow_version_id=execution.workflow_version_id,
                    workflow_execution_id=delegation.source_execution_id, action="workflow.delegation.failed", resource_type="agent_delegation",
                    resource_id=str(delegation.id), trace_id=delegation.trace_id, status="failed",
                    metadata_json={"worker_execution_id": str(worker_execution_id), "error_code": normalized_error_code}))
    db.add(WorkflowTraceEvent(tenant_id=tenant_id, execution_id=delegation.source_execution_id, workflow_id=execution.workflow_id,
                              workflow_version_id=execution.workflow_version_id, event_type="agent.delegation.failed", status="failed",
                              trace_id=delegation.trace_id, actor_id=execution.created_by,
                              data={"delegation_id": str(delegation.id), "worker_execution_id": str(worker_execution_id), "error_code": normalized_error_code}))
    await db.commit()
    await db.refresh(delegation)
    return delegation


async def timeout_delegation(*, db: AsyncSession, tenant_id: UUID, delegation_id: UUID, worker_execution_id: UUID) -> AgentDelegation:
    ...
    Args:
        ...
        tenant_id: Delegation 所属租户。
        delegation_id: Delegation 标识。
        worker_execution_id: 触发 timeout 的当前 Worker generation。

    Returns:
        AgentDelegation: 已进入 timed_out 的持久化 Delegation。

    Raises:
        HTTPException: generation 失效、Worker 已经完成或状态转换非法。

    设计意图：timeout 只结束 Delegation 自身；父 Workflow Execution 不进入 terminal 状态。
    """
    delegation, execution = await _lock_delegation(db, tenant_id=tenant_id, delegation_id=delegation_id, worker_execution_id=worker_execution_id)
    if execution.status not in {"pending", "running", "cancelled"}:
    ...
    await db.flush()
    db.add(AuditLog(actor_id=execution.created_by, tenant_id=tenant_id, workflow_id=execution.workflow_id, workflow_version_id=execution.workflow_version_id,
                    workflow_execution_id=delegation.source_execution_id, action="workflow.delegation.timed_out", resource_type="agent_delegation",
                    resource_id=str(delegation.id), trace_id=delegation.trace_id, status="timed_out",
                    metadata_json={"worker_execution_id": str(worker_execution_id)}))
    db.add(WorkflowTraceEvent(tenant_id=tenant_id, execution_id=delegation.source_execution_id, workflow_id=execution.workflow_id,
                              workflow_version_id=execution.workflow_version_id, event_type="agent.delegation.timed_out", status="timed_out",
                              trace_id=delegation.trace_id, actor_id=execution.created_by,
                              data={"delegation_id": str(delegation.id), "worker_execution_id": str(worker_execution_id)}))
    await db.commit()
    await db.refresh(delegation)
