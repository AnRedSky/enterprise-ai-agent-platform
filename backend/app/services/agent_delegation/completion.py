...
async def timeout_delegation(
    *,
    ...
    Raises:
        HTTPException: generation 失效、Worker Execution 已经成功/失败或状态转换非法。

    事务边界：Delegation 行通过 `SELECT ... FOR UPDATE` 锁定并继续验证当前 Worker generation。
    timeout 只终止 Delegation 自身；子 Worker Execution 的取消使用既有 WorkflowExecution lifecycle，
    父 Workflow Execution 不参与本事务。AuditLog 与 WorkflowTraceEvent 与 Delegation timeout
    在同一事务提交。
    """
    delegation, execution = await _lock_delegation(
        db,
