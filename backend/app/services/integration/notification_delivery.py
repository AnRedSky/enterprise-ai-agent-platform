    ...
    async def dispatch_event(
        ...
        *,
        ...
        if subscriptions:
            rows = await self.db.execute(
                select(WebhookDestination).where(
                    WebhookDestination.id.in_([s.destination_id for s in subscriptions])
                )
            )
        ...
        if provider_order:
            ...
            subscriptions.sort(
                key=lambda s: (
                    order.get(provider_by_destination.get(s.destination_id, ""), len(order)),
                    s.priority,
                    s.id,
                )
            )
        ...
        if fallback and subscriptions:
            ...
            subscriptions = [
                ...
                if provider_by_destination.get(s.destination_id) == selected_provider
            ][:1]
