...
class AlertLifecycleService:
    ...
    async def evaluate_rule(self, rule: RuntimeAlertRule, sample: RuntimeMetricSample, *,
        ...
        instance = await self.db.scalar(select(RuntimeAlertInstance).where(
            ...
            RuntimeAlertInstance.fingerprint == self.fingerprint(rule, dimensions)))
        if instance is None:
            instance = RuntimeAlertInstance(tenant_id=rule.tenant_id, rule_id=rule.id,
                fingerprint=self.fingerprint(rule, dimensions), state="inactive", severity=rule.severity,
    ...
    async def _emit_transition(self, instance: RuntimeAlertInstance, rule: RuntimeAlertRule,
                               sample: RuntimeMetricSample, transition: str, dimensions: dict[str, Any], now: datetime) -> None:
        ...
        event = await self.publisher.publish(tenant_id=instance.tenant_id, event_type=f"alert.{transition}",
            source="alert-runtime", subject=f"alert:{instance.id}",
    ...
    async def _route_notification(self, instance: RuntimeAlertInstance, event: IntegrationEventRecord, transition: str,
                                  severity: str, policy: RuntimeNotificationPolicy, now: datetime) -> None:
        ...
        for delivery in deliveries:
            ...
            self.db.add(RuntimeNotificationDelivery(tenant_id=instance.tenant_id, alert_instance_id=instance.id,
                group_id=group.id, integration_event_id=event.id, webhook_delivery_id=delivery.id,
                transition=transition, provider=provider, dedup_key=f"{instance.id}:{transition}:{group.id}:{delivery.destination_id}", status="planned"))
