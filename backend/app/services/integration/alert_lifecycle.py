...
class AlertLifecycleService:
    ...
    async def evaluate_rule(self, rule: RuntimeAlertRule, sample: RuntimeMetricSample, *,
                            ...
                            now: datetime | None = None) -> RuntimeAlertInstance:
    ...
    async def _emit_transition(self, instance: RuntimeAlertInstance, rule: RuntimeAlertRule,
                               sample: RuntimeMetricSample, transition: str,
                               dimensions: dict[str, Any], now: datetime) -> None:
        ...
        event = await self.publisher.publish(
            tenant_id=instance.tenant_id, event_type=f"alert.{transition}", source="alert-runtime",
            subject=f"alert:{instance.id}",
            idempotency_key=f"alert:{instance.id}:{transition}:{instance.fire_count}",
            payload={"alert_instance_id": str(instance.id), "rule_id": str(rule.id),
                     "metric_name": rule.metric_name, "value": sample.value, "threshold": rule.threshold,
                     "operator": rule.operator, "severity": severity, "routing_key": instance.routing_key,
                     "transition": transition, "dimensions": dimensions, "escalation_level": level},
            occurred_at=now,
        )
    ...
    def _escalate(self, base_severity: str, fire_count: int, rule: RuntimeAlertRule) -> tuple[str, int]:
        ...
        level = 0
        for item in sorted(getattr(rule, "escalation", None) or [], key=lambda value: int(value.get("after", 0))):
    ...
    async def _select_policy(self, tenant_id: uuid.UUID, severity: str, routing_key: str) -> RuntimeNotificationPolicy | None:
        ...
        for policy in policies:
            if policy.severity not in (None, severity) or policy.routing_key not in (None, routing_key):
                continue
            return policy
        return None

    async def _route_notification(self, instance: RuntimeAlertInstance, event: IntegrationEventRecord,
                                  transition: str, severity: str, policy: RuntimeNotificationPolicy,
                                  now: datetime) -> None:
        ...
        if group is None or group.closed_at is not None or now - group.last_event_at > timedelta(seconds=policy.group_window_seconds):
            group = RuntimeNotificationGroup(tenant_id=instance.tenant_id, group_key=group_key,
                severity=severity, routing_key=instance.routing_key, first_event_at=now, last_event_at=now)
            self.db.add(group)
        else:
        ...
        deliveries = await self.delivery.dispatch_event(event, destination_ids=destinations,
            provider_order=policy.provider_order, fallback=True)
        for delivery in deliveries:
            ...
            record = RuntimeNotificationDelivery(
                ...
                integration_event_id=event.id, webhook_delivery_id=delivery.id, transition=transition,
                provider=provider, dedup_key=f"{instance.id}:{transition}:{group.id}:{delivery.destination_id}", status="planned")
            self.db.add(record)
            await self._audit(instance, "notification.route", "success", {
                "transition": transition, "delivery_id": str(delivery.id), "provider": provider,
                "group_id": str(group.id), "severity": severity}, now)
    ...
    async def record_delivery_outcome(self, webhook_delivery_id: uuid.UUID, *, status: str,
        ...
        """Synchronize Worker terminal/retry outcome into notification facts and metrics."""
        ...
        record = await self.db.scalar(select(RuntimeNotificationDelivery).where(
            RuntimeNotificationDelivery.tenant_id == RuntimeNotificationDelivery.tenant_id,
        ...
        await self._audit_for_delivery(record, f"notification.delivery.{status}", "failure" if status in {"failed", "dead_letter"} else "success", now)
    ...
    async def _metric(self, tenant_id: uuid.UUID, metric_name: str, value: float,
                      dimensions: dict[str, Any], now: datetime) -> None:
        self.db.add(RuntimeMetricSample(tenant_id=tenant_id, metric_name=metric_name,
            value=value, dimensions=dimensions, recorded_at=now))

    async def _audit(self, instance: RuntimeAlertInstance, action: str, outcome: str,
                     details: dict[str, Any], now: datetime) -> None:
        self.db.add(RuntimeOperationAudit(tenant_id=instance.tenant_id, actor=self.actor,
            action=action, resource_type="alert", resource_id=str(instance.id), outcome=outcome,
