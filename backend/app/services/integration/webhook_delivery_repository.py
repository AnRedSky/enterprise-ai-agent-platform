...
class WebhookDeliveryRepository:
    ...
    async def claim_next(self, db: AsyncSession, now: datetime, owner: str, lease_seconds: int = 60, max_attempts: int = 5) -> WebhookDelivery | None:
        if not owner.strip():
    ...
    async def mark_delivered(self, db: AsyncSession, delivery_id: uuid.UUID, owner: str, now: datetime, status_code: int) -> bool:
        result = await db.execute(select(WebhookDelivery).where(WebhookDelivery.id == delivery_id, WebhookDelivery.status == "running", WebhookDelivery.lease_owner == owner).with_for_update())
        record = result.scalar_one_or_none()
        if record is None:
            return False
    ...
    async def mark_failed(self, db: AsyncSession, delivery_id: uuid.UUID, owner: str, now: datetime, error_code: str, error_message: str, retry_at: datetime | None) -> bool:
        result = await db.execute(select(WebhookDelivery).where(WebhookDelivery.id == delivery_id, WebhookDelivery.status == "running", WebhookDelivery.lease_owner == owner).with_for_update())
        record = result.scalar_one_or_none()
        if record is None:
            return False
    ...
    async def replay(self, db: AsyncSession, tenant_id: uuid.UUID, delivery_id: uuid.UUID, actor: str) -> WebhookDelivery | None:
        result = await db.execute(select(WebhookDelivery).where(WebhookDelivery.id == delivery_id, WebhookDelivery.tenant_id == tenant_id).with_for_update())
        record = result.scalar_one_or_none()
        if record is None:
            return None
