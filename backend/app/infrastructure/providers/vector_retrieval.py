...
class VectorRetrievalProvider(Protocol):
    ...
    async def search(self, query_embedding: Sequence[float], top_k: int, min_score: float = 0.0, knowledge_base_id: str | None = None) -> list[VectorSearchResult]: ...


class InMemoryVectorRetrievalProvider:
    """Deterministic local adapter for provider contract tests."""
    ...
    async def search(self, query_embedding: Sequence[float], top_k: int, min_score: float = 0.0, knowledge_base_id: str | None = None) -> list[VectorSearchResult]:
        ...
        for record in self._records.values():
            ...
            if score >= min_score:
                results.append(VectorSearchResult(chunk_id=record.chunk_id, score=round(score, 6), metadata=record.metadata))
...
class PgVectorRetrievalProvider:
        ...
        for record in records:
            ...
            if evaluation:
                await self.db.execute(text(f"""
                    INSERT INTO {table} (chunk_id, knowledge_base_id, embedding_dimension, embedding, metadata)
                    VALUES (:chunk_id, CAST(:knowledge_base_id AS uuid), :embedding_dimension, CAST(:embedding AS vector), CAST(:metadata AS jsonb))
                    ...
                    """), {"chunk_id": record.chunk_id, "knowledge_base_id": knowledge_base_id, "embedding_dimension": self.embedding_dimension, "embedding": self._vector_literal(record.embedding), "metadata": json.dumps(record.metadata)})
                continue
            if not document_version_id:
                raise VectorRetrievalProviderError("pgvector record metadata requires knowledge_base_id and document_version_id")
            await self.db.execute(text(f"""
                INSERT INTO {table} (chunk_id, knowledge_base_id, document_version_id, embedding, metadata)
                VALUES (:chunk_id, CAST(:knowledge_base_id AS uuid), CAST(:document_version_id AS uuid), CAST(:embedding AS vector), CAST(:metadata AS jsonb))
                ON CONFLICT (chunk_id) DO UPDATE SET
                    knowledge_base_id = EXCLUDED.knowledge_base_id, document_version_id = EXCLUDED.document_version_id,
                    embedding = EXCLUDED.embedding, metadata = EXCLUDED.metadata, updated_at = CURRENT_TIMESTAMP
                """), {"chunk_id": record.chunk_id, "knowledge_base_id": knowledge_base_id, "document_version_id": document_version_id, "embedding": self._vector_literal(record.embedding), "metadata": json.dumps(record.metadata)})
    ...
    async def search(self, query_embedding: Sequence[float], top_k: int, min_score: float = 0.0, knowledge_base_id: str | None = None) -> list[VectorSearchResult]:
        ...
        table = self.EVALUATION_TABLE if evaluation_scope else self.TABLE
        dimension_clause = "AND embedding_dimension = :embedding_dimension" if evaluation_scope else ""
        result = await self.db.execute(text(f"""
            SELECT chunk_id::text AS chunk_id, 1 - (embedding <=> CAST(:embedding AS vector)) AS score, metadata
            FROM {table}
            ...
            ORDER BY embedding <=> CAST(:embedding AS vector) ASC, chunk_id ASC LIMIT :top_k
            """), {"embedding": self._vector_literal(query_embedding), "knowledge_base_id": knowledge_base_id, "embedding_dimension": self.embedding_dimension, "min_score": min_score, "top_k": top_k})
        return [VectorSearchResult(chunk_id=str(row.chunk_id), score=round(float(row.score), 6), metadata=dict(row.metadata or {})) for row in result.fetchall()]

    async def _has_evaluation_vectors(self, knowledge_base_id: str | None) -> bool:
        if knowledge_base_id is None:
            return False
        result = await self.db.execute(text(f"""
            SELECT EXISTS(SELECT 1 FROM {self.EVALUATION_TABLE}
            WHERE knowledge_base_id = CAST(:knowledge_base_id AS uuid) AND embedding_dimension = :embedding_dimension)
            """), {"knowledge_base_id": knowledge_base_id, "embedding_dimension": self.embedding_dimension})
    ...
    def _validate_embedding(self, embedding: Sequence[float]) -> None:
        ...
        if len(embedding) != self.embedding_dimension:
            raise VectorRetrievalProviderError(f"embedding dimensions must match configured dimension {self.embedding_dimension}")
