...
async def list_executions(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), status: str | None = None, agent_id: UUID | None = None, trace_id: str | None = None, request_id: str | None = None, session_id: UUID | None = None, started_from: datetime | None = None, started_to: datetime | None = None, db: AsyncSession = Depends(get_db)):
    actor_id, is_admin = _identity()
    page, page_size, total, rows = await RuntimeQueryService(db).executions(actor_id, is_admin, page, page_size, status, agent_id, trace_id, request_id, session_id, started_from, started_to)
...
async def execution_detail(execution_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    return {"execution": execution, "items": []}
...
async def list_audit_logs(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), agent_id: UUID | None = None, tool_id: UUID | None = None, status: str | None = None, db: AsyncSession = Depends(get_db)):
    actor_id, is_admin = _identity()
