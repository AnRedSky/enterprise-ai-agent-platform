...
async def runtime_operation_audit_query(page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=100), action: str | None = Query(None, min_length=1, max_length=80), resource_type: str | None = Query(None, min_length=1, max_length=80), resource_id: str | None = Query(None, min_length=1, max_length=128), outcome: str | None = Query(None, min_length=1, max_length=24), actor: Annotated[str | None, StringConstraints(min_length=1, max_length=128), Query()] = None, since: datetime | None = Query(None), until: datetime | None = Query(None), claims: dict = Depends(_claims), db: AsyncSession = Depends(get_db)):
