...
$register = Invoke-ScenarioRequest `
    -Name "Auth / register" `
    -Method "POST" `
...
# Runtime: list executions and inspect the execution generated by Chat when available.
$executions = Invoke-ScenarioRequest -Name "Runtime / executions" -Path "/api/v1/runtime/executions" -Headers $headers
if ($executions -is [System.Array] -and $executions.Count -gt 0) {
    $executionId = [string]$executions[0].execution_id
