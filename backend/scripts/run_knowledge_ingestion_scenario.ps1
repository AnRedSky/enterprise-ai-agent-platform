...
function Invoke-ScenarioRequest {
    param([string]$Name,[string]$Method="GET",[string]$Path,[hashtable]$Headers=@{},[object]$Body=$null,[int[]]$ExpectedStatus=@(200))
    ...
    try {
        ...
        if ($null -ne $status -and ($ExpectedStatus -contains $status)) { Write-Host "[ OK  ] $Name -> HTTP $status" -ForegroundColor Green; return $null }
...
if ([string]::IsNullOrWhiteSpace($Username)) { $Username="ingestion_$(Get-Date -Format 'yyyyMMddHHmmssfff')" }
...
$ingest=Invoke-ScenarioRequest -Name "Knowledge / ingest" -Method POST -Path "/api/v1/knowledge/versions/$versionId/ingest" -Headers $headers -Body @{ max_chars=80; overlap_chars=10 }
...
$ingestAgain=Invoke-ScenarioRequest -Name "Knowledge / re-ingest" -Method POST -Path "/api/v1/knowledge/versions/$versionId/ingest" -Headers $headers -Body @{ max_chars=80; overlap_chars=10 }
