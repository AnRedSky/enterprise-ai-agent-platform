...
Write-Host '============================================================'
Write-Host 'Enterprise AI Agent Platform - Durable Resume Failure Acceptance'
Write-Host '============================================================'
...
function Assert-ApiAvailable {
    ...
    try {
        ...
        if ($response.StatusCode -ne 200) {
            throw "Workflow API health check returned HTTP $($response.StatusCode)."
        }
    } catch {
...
try {
    ...
    Write-Host '[4/4] Execute real PostgreSQL + independent Worker failure-after-resume acceptance'
    & uv run pytest -q tests/api_real/test_workflow_resume_failure_api.py -m real_api
    if ($LASTEXITCODE -ne 0) {
        throw 'Durable Resume failure-after-resume acceptance failed.'
    }

    Write-Host '[PASS] Durable Resume failure-after-resume acceptance completed.'
    Write-Host '[INFO] Source remains failed; Resume keeps source lineage and original Workflow Version; failed Resume does not create a false completion checkpoint.'
} finally {
    Pop-Location
