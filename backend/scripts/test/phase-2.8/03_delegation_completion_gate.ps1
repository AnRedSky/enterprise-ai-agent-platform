...
function Ensure-Infrastructure {
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        throw "Docker is required to run the Phase 2.8 B3 acceptance gate."
    }
...
function Ensure-ApiService {
    ...
    try {
        ...
        $stdout = Join-Path $logDir "b3-api.stdout.log"
        $stderr = Join-Path $logDir "b3-api.stderr.log"
...
function Get-AutomatedAccessToken([string]$ApiUrl) {
    ...
    $username = "phase28-b3-$suffix"
    $password = "Phase28B3-$suffix-A9!"
    ...
    if (-not $login.access_token) { throw "Automated B3 test-user login returned no access_token." }
    return $login.access_token
}

try {
    Write-Host "============================================================"
    Write-Host "Enterprise AI Agent Platform - Phase 2.8 B3 Delegation Completion Gate"
    Write-Host "============================================================"

    Write-Host "[1/4] Delegation lifecycle Unit"
    uv run pytest -q tests/unit/test_agent_delegation_identity.py tests/unit/test_agent_delegation_lifecycle.py
    Assert-ExitCode "Delegation lifecycle unit gate failed."
    ...
    Write-Host "[4/4] Real HTTP + PostgreSQL B3 completion/fencing"
    ...
    Assert-ExitCode "B3 Delegation completion/fencing real acceptance failed."

    Write-Host "[PASS] Phase 2.8 B3 Delegation completion/fencing gate completed."
} finally {
