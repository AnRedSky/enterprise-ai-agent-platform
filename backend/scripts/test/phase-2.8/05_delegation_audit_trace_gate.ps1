...
function Assert-PrerequisiteServices {
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        throw "Docker is required to run the Phase 2.8 B5 acceptance gate."
    }
...
function Get-AutomatedAccessToken([string]$ApiUrl) {
    ...
    $username = "phase28-b5-$suffix"
    $password = "Phase28B5-$suffix-A9!"
    ...
    if (-not $login.access_token) { throw "Automated B5 test-user login returned no access_token." }
    return $login.access_token
}

try {
    Write-Host "============================================================"
    Write-Host "Enterprise AI Agent Platform - Phase 2.8 B5 Delegation Audit/Trace Gate"
    Write-Host "============================================================"

    Write-Host "[0/4] Local prerequisite service verification (no service startup)"
    Assert-PrerequisiteServices

    Write-Host "[1/4] Worker shutdown + Delegation lifecycle Unit"
    uv run pytest -q tests/unit/test_worker_entrypoint.py tests/unit/test_agent_delegation_lifecycle.py tests/unit/test_agent_delegation_timeout.py
    Assert-ExitCode "B5 unit gate failed."
    ...
    Write-Host "[4/4] Real HTTP + PostgreSQL Delegation Audit/Trace closure"
    $ApiBaseUrl = "http://127.0.0.1:8000/api/v1"
    $env:API_BASE_URL = $ApiBaseUrl
    $env:ACCESS_TOKEN = Get-AutomatedAccessToken $ApiBaseUrl
    uv run pytest -q -o "addopts=" -m real_api tests/api_real/test_agent_delegation_b5_audit_trace_api.py tests/api_real/test_agent_delegation_b4_api.py
    Assert-ExitCode "B5 Delegation Audit/Trace real acceptance failed."

    Write-Host "[PASS] Phase 2.8 B5 Delegation Audit/Trace gate completed."
} finally {
