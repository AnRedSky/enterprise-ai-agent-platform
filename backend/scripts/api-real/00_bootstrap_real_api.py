...
def main() -> int:
    ...
    with httpx.Client(base_url=BASE_URL, timeout=TIMEOUT) as client:
        ...
        if workflow_id is None:
            ...
            version = request(client, "POST", f"/workflows/{workflow_id}/versions", json={
                "definition": {"nodes": [], "edges": []}
            }).json()
            request(client, "POST", f"/workflows/{workflow_id}/versions/{version['id']}/publish")

        execution = request(client, "POST", f"/workflows/{workflow_id}/executions", json={
            "input_data": {"source": "real_api_validation"}
        }).json()
