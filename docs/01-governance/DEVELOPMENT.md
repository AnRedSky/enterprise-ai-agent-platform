...
涉及当前时间、租约、TTL、超时、misfire、cooldown、调度窗口或其他时间边界的测试，不得使用已经过期的固定时间作为“有效”测试数据。测试应优先使用相对当前时间生成未来窗口；若业务需要验证过期分支，则显式生成过去窗口。对于复杂时间规则，生产代码优先提供可控时钟或时间注入点，测试不得依赖自然时间流逝才能稳定通过。

测试夹具必须与被测规则保持一致：如果生产代码会按当前时间校验 lease，测试夹具必须明确构造“有效 lease”和“过期 lease”两类事实，不得用历史日期同时承担两种语义。发现此类测试误差时，应记录根因并补充反向边界测试。
...
测试 Gate 可以集成服务状态探测、健康检查、进程识别以及明确的人工启动命令提示，但默认禁止自动创建、启动、重启或停止 API、Worker、Scheduler、PostgreSQL、Redis 等服务进程。这样可以避免测试抢占开发者现有端口、污染运行态或在未知配置下启动错误实例。

依赖服务缺失时，Gate 必须明确输出“未执行”，同时给出无需人工填写测试数据的标准启动命令；禁止把缺少服务伪装成测试失败，也禁止要求开发者手工修改测试源码、fixture 或 ID 才能继续。服务检查通过后，测试脚本自动生成或读取所需测试上下文。
...
36. **时间敏感单元测试不得把固定历史时间误用为有效租约、有效 TTL 或未来调度窗口。优先使用相对当前时间的测试夹具，或对生产时钟进行显式注入/冻结；同时必须至少覆盖有效、边界、过期三类时间状态。**
37. **测试失败诊断必须先使用最小可复现 Gate：优先运行失败测试文件或单测试，再使用 `-x --tb=long` 定位首个失败；首个失败修复并验证后再恢复全量回归。禁止在首个确定性失败尚未修复时反复运行长时间 Real API/E2E 流程。**
38. **服务依赖型测试脚本可以包含“启动命令说明”和健康检查逻辑，但不得在默认路径自动启动、重启或停止任何服务，也不得要求手工填写测试 ID、Token 或上下文参数。测试所需上下文应由 bootstrap 脚本自动生成并在 Gate 生命周期内清理。**
39. **任何由测试夹具、测试脚本或环境基线造成的确定性误报，都必须与生产代码缺陷区分记录；修复测试误报时不得弱化生产校验。必须补充对应的反向边界测试，确保原有安全/一致性约束仍被覆盖。**
