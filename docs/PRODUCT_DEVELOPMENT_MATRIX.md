...
| Runtime | 稳定执行 Agent | Runtime + Session + Context + Model/Tool/Knowledge/Memory | 已验收 | Profile resolution 尚需接入 | 2.2-E 后续 |
| Model Gateway | Provider-neutral LLM 接入 | Mock/OpenAI-compatible、普通/流式 Contract | 已覆盖当前范围 | Model Profile runtime selection、路由/Fallback/成本治理待后续产品决策 | 先完成 2.2-E Profile 接入 |
| Model Provider / Profile | 模型供应商与模型身份可配置、选择、追踪、评估 | Provider/Profile 数据模型、CRUD API、Organization scope、Audit | 工程实现中 | Runtime/Evaluation 尚未完成 profile selection | **2.2-E** |
...
| Observability | 可查询、可追踪、可审计 | Execution/Event/Trace/Audit + UI | 已验收当前范围 | Model Profile identity 尚需写入 Runtime trace | 2.2-E 后续 |
| Knowledge | 企业文档知识库 | KB/Document/Version/Chunk/Ingestion | 已覆盖当前范围 | 真实生产质量需验证 | Phase 2.2 联动 |
| Retrieval | 高质量企业检索 | Lexical/Vector/Hybrid/Citation/Debug/Evaluation 工程链路 | 已覆盖工程链路 | 真实语义质量仍需产品目标确认 | Phase 2.2 |
| Workflow | 可治理流程执行 | Definition/Version/Publish/Execution | 已验收 | 复杂 DAG/Saga/Engine 未实现 | Phase 2.5 候选 |
| Governance | Tenant/RBAC/Audit/Trace/Reliability | 已形成闭环 | 已验收 | 更完整模型治理逐步补齐 | 2.2-E / 2.3 后续 |
...
| Frontend | 管理、配置、调试 | Vue 3 + API Types + Governance UI | 已验收当前范围 | Provider/Profile 管理 UI 待 2.2-E 后续 | 2.2-E 后续 |
| Browser E2E | 真实用户链路验证 | Playwright Browser → Vue → Backend | 已验收 | 新 Provider/Profile UI 需独立 E2E | 随 UI 进入 |
...
| **2.2** | **Retrieval Production Quality + 2.2-E Model Provider/Profile Foundation** | **进行中** | **是，当前阶段** |
| 2.3 | Model Provider Governance（路由/Fallback/成本/用量） | 候选路线；本阶段不提前实现 | 需求确认后 |
...
2.2-E 当前只达到 P0/P1/P2 设计与自动化 Contract 覆盖；未执行 Real API / Browser 的结果不得标记为 Passed。
...
当前已建立 Provider/Profile 数据模型、Organization scoped CRUD、credential reference 与 Audit；缺少 Runtime / Evaluation 的 `model_profile_id` 实际解析和 trace identity。
...
2. Provider identity 与 adapter type 分离。
3. Credential reference 边界。
4. Chat / Embedding Profile schema。
5. Embedding dimension contract。
6. Default Profile 语义。
7. Provider/Profile CRUD API、权限、Audit。
8. Migration 与 API Contract tests。
9. Runtime / Evaluation Profile Resolution 后续接入契约。
...
当前正式工作保持在 **Phase 2.2**。2.2-E 是为 Retrieval evaluation 与 Agent Runtime 建立受治理模型身份的基础设施，不等同提前启动完整 Phase 2.3。Phase 2.3 的 Provider routing、Fallback、成本/用量治理仍需独立产品 Contract 后再进入开发。