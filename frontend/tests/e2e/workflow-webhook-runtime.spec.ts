...
async function assertTriggerStatus(api: APIRequestContext, workflowId: string, triggerId: string, headers: Record<string, string>, status: string) {
  await expect.poll(async () => {
    const response = await api.get(apiPath(`/workflows/${workflowId}/triggers`), { headers });
    expect(response.ok()).toBeTruthy();
    const body = await response.json();
    const items = Array.isArray(body) ? body : body.items;
    return items?.find((item: { id: string }) => item.id === triggerId)?.status;
...
test("Webhook browser runtime converges duplicate events and enforces lifecycle security", async ({ page, playwright }) => {
  ...
  try {
    ...
    const workflowResponse = await api.post(apiPath("/workflows"), { headers, data: { name: `Browser Webhook Runtime ${nonce}`, description: "Phase 1.8-E browser runtime fixture" } });
    expect(workflowResponse.status()).toBe(201);
    const workflow = await workflowResponse.json();
    const versionResponse = await api.post(apiPath(`/workflows/${workflow.id}/versions`), { headers, data: { definition: { nodes: [{ id: "input", type: "input", config: {} }, { id: "output", type: "output", config: {} }], edges: [{ source: "input", target: "output" }] } } });
    ...
    const workflowOption = page.locator(".el-select-dropdown:visible .el-select-dropdown__item").filter({ hasText: `${workflow.name} (published)` }).first();
    await expect(workflowOption).toBeVisible();
    await workflowOption.click();
    await page.getByLabel("Trigger 名称").fill(triggerName);
    await page.locator(".el-form-item").filter({ hasText: "类型" }).first().locator(".el-select").click();
    ...
    const first = await api.post(webhookPath, { headers: { "X-Webhook-Secret": secret, "Idempotency-Key": eventId }, data: payload });
    ...
    const duplicate = await api.post(webhookPath, { headers: { "X-Webhook-Secret": secret, "Idempotency-Key": eventId }, data: payload });
    expect(duplicate.status()).toBe(200);
    expect(await duplicate.json()).toMatchObject({ status: "duplicate", execution_id: firstBody.execution_id, idempotency_key: firstBody.idempotency_key });
    const invalidSecret = await api.post(webhookPath, { headers: { "X-Webhook-Secret": `${secret}-invalid`, "Idempotency-Key": `invalid-${eventId}` }, data: payload });
    ...
    }, { timeout: 20_000, intervals: [500, 1000, 2000] }).toMatchObject({ id: firstBody.execution_id, status: expect.any(String) });

    await triggerRow.getByRole("button", { name: "禁用" }).click();
    await assertTriggerStatus(api, workflow.id, persistedTrigger.id, headers, "disabled");
    const disabled = await api.post(webhookPath, { headers: { "X-Webhook-Secret": secret, "Idempotency-Key": `disabled-${eventId}` }, data: { ...payload, event_id: `disabled-${eventId}` } });
    ...
    const deleted = await api.post(webhookPath, { headers: { "X-Webhook-Secret": secret, "Idempotency-Key": `deleted-${eventId}` }, data: { ...payload, event_id: `deleted-${eventId}` } });
