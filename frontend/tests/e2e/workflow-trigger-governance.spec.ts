...
test("Workflow Trigger Governance completes the real scheduled browser contract", async ({ page, playwright }) => {
  ...
  try {
    ...
    await page.goto("/workflows/triggers"); await expect(page.getByText("Workflow Trigger Governance", { exact: true })).toBeVisible(); const triggerName = `Scheduled Trigger ${unique}`; await page.getByLabel("Trigger 名称").fill(triggerName); await page.getByTestId("workflow-trigger-type-select").click(); await page.getByRole("option", { name: "scheduled" }).click(); await page.getByRole("button", { name: "创建 Trigger" }).click();
    ...
    const schedulerCard = page.locator(".scheduler-card"); await expect(schedulerCard).toBeVisible(); const schedulerRefresh = schedulerCard.getByRole("button", { name: "刷新" }); if (await schedulerCard.getByTestId("scheduler-timezone").count() === 0) await schedulerRefresh.click(); await expect(schedulerCard.getByTestId("scheduler-timezone")).toHaveText("UTC", { timeout: 15_000 }); await expect(schedulerCard.getByTestId("scheduler-misfire-policy")).toHaveText("skip"); await expect(schedulerCard.getByTestId("scheduler-catch-up-limit")).toHaveText("10");
    await triggerRow.getByRole("button", { name: "禁用" }).click(); await expect(triggerRow).toContainText("disabled"); await waitForTriggerStatus(api, workflow.id, persistedCreatedTrigger.id, headers, "disabled"); await triggerRow.getByRole("button", { name: "删除" }).click(); await page.locator(".el-message-box:visible .el-message-box__btns .el-button--primary").click(); await expect(page.getByText("Trigger 已删除")).toBeVisible(); await expect(page.getByRole("row").filter({ hasText: triggerName })).toHaveCount(0);
