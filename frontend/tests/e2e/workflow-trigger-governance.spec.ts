...
async function waitForPersistedTrigger(api: APIRequestContext, workflowId: string, triggerName: string, headers: Record<string, string>) {
  ...
  while (Date.now() < deadline) {
    ...
    const trigger = lastItems.find((item): item is { name: string; id: string } => typeof item === "object" && item !== null && "name" in item && item.name === triggerName);
...
test("Workflow Trigger Governance completes the real scheduled browser contract", async ({ page, playwright }) => {
  ...
  try {
    ...
    const workflowResponse = await api.post(apiPath("/workflows"), { headers, data: { name: `E2E Scheduled Workflow ${unique}`, description: "Browser E2E scheduler persistence contract" } });
    expect(workflowResponse.status()).toBe(201);
    const workflow = await workflowResponse.json();
    const versionResponse = await api.post(apiPath(`/workflows/${workflow.id}/versions`), {
      headers,
      data: { definition: { nodes: [{ id: "input", type: "input", config: {} }, { id: "output", type: "output", config: {} }], edges: [{ source: "input", target: "output" }] } },
    });
    ...
    await page.addInitScript(({ token: accessToken, roles, userId, tokenKey, rolesKey, userIdKey }) => {
      localStorage.setItem(tokenKey, accessToken);
      localStorage.setItem(rolesKey, JSON.stringify(roles || []));
      localStorage.setItem(userIdKey, userId);
    }, { token, roles: loginBody.roles || [], userId: loginBody.user_id, tokenKey: accessTokenStorageKey, rolesKey: rolesStorageKey, userIdKey: userIdStorageKey });
    ...
    expect(persistedCreatedTrigger).toMatchObject({ trigger_type: "scheduled", status: "enabled", config: { timezone: "UTC", interval_seconds: 60, misfire_policy: "skip", catch_up_limit: 10 } });
    ...
    }, { timeout: 15_000, intervals: [500, 1000, 2000] }).toMatchObject({ trigger_id: persistedCreatedTrigger.id, workflow_id: workflow.id, tenant_id: workflow.tenant_id, status: "enabled", timezone: "UTC", misfire_policy: "skip", catch_up_limit: 10, lease_active: expect.any(Boolean) });
    ...
    if (await schedulerCard.getByTestId("scheduler-timezone").count() === 0) await schedulerRefresh.click();
    ...
    await expect.poll(async () => (await api.get(apiPath(`/workflows/${workflow.id}/triggers`), { headers })).json()).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ id: persistedCreatedTrigger.id, status: "disabled" })]));
    ...
    await expect(page.getByRole("row").filter({ hasText: triggerName })).toHaveCount(0);
  } finally {
    await api.dispose();
  }
});