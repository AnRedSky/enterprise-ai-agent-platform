...
test("Workflow Trigger Governance completes the real scheduled browser contract", async ({ page, playwright }) => {
  ...
  try {
    ...
    const triggerName = `Scheduled Trigger ${unique}`; await page.getByLabel("Trigger 名称").fill(triggerName); await page.getByTestId("workflow-trigger-type-select").click(); await page.getByRole("option", { name: "scheduled", exact: true }).click(); await page.getByRole("button", { name: "创建 Trigger" }).click();
    ...
    await triggerRow.getByRole("button", { name: "调度状态" }).click(); const schedulerCard = page.locator(".ui-surface-card").filter({ has: page.getByRole("heading", { name: "Scheduler 持久化状态", exact: true }) }); await expect(schedulerCard).toBeVisible(); await expect(schedulerCard.getByText("时区", { exact: true })).toBeVisible(); await expect(schedulerCard.getByText("UTC", { exact: true })).toBeVisible(); await expect(schedulerCard.getByText("Misfire", { exact: true })).toBeVisible(); await expect(schedulerCard.getByText("skip", { exact: true })).toBeVisible(); await expect(schedulerCard.getByText("Catch-up Limit", { exact: true })).toBeVisible(); await expect(schedulerCard.getByText("10", { exact: true })).toBeVisible();
