...
test("Workflow Trigger Governance completes the real webhook browser contract", async ({ page, playwright }) => {
  ...
  try {
    ...
    const workflowResponse = await api.post(apiPath("/workflows"), { headers, data: { name: `Browser Webhook E2E ${nonce}`, description: "Phase 1.8-C browser fixture" } });
    ...
    expect(persistedTrigger).toMatchObject({ trigger_type: "webhook", status: "enabled", config: { auth_mode: "secret", event_id_field: "event_id", secret_configured: true } });
    ...
    await waitForTriggerStatus(api, workflow.id, persistedTrigger.id, headers, "disabled");
