...
test("Organization management completes the real owner browser contract", async ({ page, playwright }) => {
  ...
  try {
    const ownerLogin = await loginOwner(api); const ownerBody = await ownerLogin.json(); const headers = { Authorization: `Bearer ${ownerBody.access_token}` }; const organization = await createOrganization(api, headers, `Organization E2E ${nonce}`);
    ...
    const addMember = await api.post(apiPath(`/organizations/${organization.id}/members`), { headers, data: { user_id: memberUser.user_id, role: "member" } }); expect(addMember.status()).toBe(201); const member = await getMembership(api, organization.id, memberUser.user_id, headers);
    await loginInBrowser(page, ownerUsername, ownerPassword); await page.goto("/organizations"); await expect(page.getByRole("heading", { name: "组织", exact: true })).toBeVisible(); await expect(page.getByRole("link", { name: "管理成员" })).toBeVisible(); await page.getByRole("link", { name: "管理成员" }).first().click(); await expect(page.getByRole("heading", { name: organization.name })).toBeVisible(); await expect(page.getByRole("heading", { name: "成员", exact: true })).toBeVisible();
    ...
    const newOwnerLogin = await api.post(apiPath("/auth/login"), { data: { username: memberUsername, password: memberPassword } }); expect(newOwnerLogin.ok()).toBeTruthy(); const newOwnerBody = await newOwnerLogin.json(); const newOwnerHeaders = { Authorization: `Bearer ${newOwnerBody.access_token}` }; const originalOwnerMembership = await getMembership(api, organization.id, ownerBody.user_id, newOwnerHeaders); const restore = await api.post(apiPath(`/organizations/${organization.id}/members/${originalOwnerMembership.id}/transfer-owner`), { headers: newOwnerHeaders }); expect(restore.ok()).toBeTruthy();
...
test("Organization browser governance enforces member and suspended-member boundaries", async ({ page, playwright }) => {
  ...
  try {
    const ownerLogin = await loginOwner(api); const ownerBody = await ownerLogin.json(); const ownerHeaders = { Authorization: `Bearer ${ownerBody.access_token}` }; const organization = await createOrganization(api, ownerHeaders, `Organization Boundary E2E ${nonce}`);
    const memberRegister = await api.post(apiPath("/auth/register"), { data: { username: memberUsername, password: memberPassword } }); expect([200, 201]).toContain(memberRegister.status()); const memberUser = await memberRegister.json(); const addMember = await api.post(apiPath(`/organizations/${organization.id}/members`), { headers: ownerHeaders, data: { user_id: memberUser.user_id, role: "member" } }); expect(addMember.status()).toBe(201); const membership = await getMembership(api, organization.id, memberUser.user_id, ownerHeaders);
    ...
    const suspend = await api.patch(apiPath(`/organizations/${organization.id}/members/${membership.id}`), { headers: ownerHeaders, data: { status: "suspended" } }); expect(suspend.ok()).toBeTruthy(); await page.reload(); await expect(page.getByRole("status")).toContainText("组织详情加载失败"); await expect(page.getByRole("status")).toContainText("当前用户无权访问该组织"); await expect(page.getByRole("heading", { name: organization.name })).toHaveCount(0);
  } finally { await api.dispose(); }
});
...
test("Organization owner transfer exposes owner-only browser controls", async ({ page, playwright }) => {
  ...
  try {
    const oldLogin = await loginOwner(api); const oldBody = await oldLogin.json(); const oldHeaders = { Authorization: `Bearer ${oldBody.access_token}` }; const organization = await createOrganization(api, oldHeaders, `Organization Transfer E2E ${nonce}`);
    const newRegister = await api.post(apiPath("/auth/register"), { data: { username: newOwnerUsername, password: newOwnerPassword } }); expect([200, 201]).toContain(newRegister.status()); const newUser = await newRegister.json(); const addMember = await api.post(apiPath(`/organizations/${organization.id}/members`), { headers: oldHeaders, data: { user_id: newUser.user_id, role: "member" } }); expect(addMember.status()).toBe(201); const membership = await getMembership(api, organization.id, newUser.user_id, oldHeaders); const transfer = await api.post(apiPath(`/organizations/${organization.id}/members/${membership.id}/transfer-owner`), { headers: oldHeaders }); expect(transfer.ok()).toBeTruthy();
    ...
    const newOwnerLogin = await api.post(apiPath("/auth/login"), { data: { username: newOwnerUsername, password: newOwnerPassword } }); expect(newOwnerLogin.ok()).toBeTruthy(); const newOwnerBody = await newOwnerLogin.json(); const newOwnerHeaders = { Authorization: `Bearer ${newOwnerBody.access_token}` }; const originalOwnerMembership = await getMembership(api, organization.id, oldBody.user_id, newOwnerHeaders); const restore = await api.post(apiPath(`/organizations/${organization.id}/members/${originalOwnerMembership.id}/transfer-owner`), { headers: newOwnerHeaders }); expect(restore.ok()).toBeTruthy();
  } finally { await api.dispose(); }
});
