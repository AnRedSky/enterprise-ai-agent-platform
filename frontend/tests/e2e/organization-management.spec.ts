...
async function getOrganization(api: APIRequestContext, headers: Record<string, string>) {
...
async function getMembership(api: APIRequestContext, organizationId: string, userId: string, headers: Record<string, string>) {
  const response = await api.get(apiPath(`/organizations/${organizationId}/members`), { headers });
  expect(response.ok()).toBeTruthy();
  const body = await response.json();
...
test("Organization management completes the real owner browser contract", async ({ page, playwright }) => {
  ...
  try {
    ...
    const organization = await getOrganization(api, headers);

    await loginInBrowser(page, username, password);
    await page.goto("/organizations");
    await expect(page.getByRole("heading", { name: "组织" })).toBeVisible();
    await expect(page.getByRole("link", { name: "管理成员" })).toBeVisible();
    await page.getByRole("link", { name: "管理成员" }).first().click();
...
test("Organization browser governance enforces member and suspended-member boundaries", async ({ page, playwright }) => {
  ...
  try {
    ...
    const organization = await getOrganization(api, ownerHeaders);
    ...
    await page.reload();
    await expect(page.getByRole("alert")).toContainText("组织详情加载失败");
...
test("Organization owner transfer exposes owner-only browser controls", async ({ page, playwright }) => {
  ...
  try {
    ...
    const organization = await getOrganization(api, oldHeaders);
    const membership = await getMembership(api, organization.id, newUser.user_id, oldHeaders);
