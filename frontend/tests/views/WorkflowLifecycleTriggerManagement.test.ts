...
function mountPage() {
  return mount(WorkflowLifecycle, {
    global: {
      ...
      stubs: {
        PageHeader: true, SurfaceCard: true, StatePanel: true, ConfirmDialog: true, "el-button": true, "el-select": true, "el-option": true, "el-tag": true, "el-descriptions": true, "el-descriptions-item": true, "el-empty": true, "el-table": true, "el-table-column": true, "el-alert": true, "el-dialog": true, "el-form": true, "el-form-item": true, "el-input": true, "el-input-number": true,
      },
    },
  });
}
...
describe("WorkflowLifecycle trigger management", () => {
  ...
  it("opens scheduled Trigger editor with backend durable configuration", async () => {
    ...
    expect((wrapper.vm as any).triggerEditor.trigger.id).toBe("t-scheduled");
    expect((wrapper.vm as any).triggerEditor.timezone).toBe("Asia/Seoul");
    expect((wrapper.vm as any).triggerEditor.interval_seconds).toBe(600);
    expect((wrapper.vm as any).triggerEditor.misfire_policy).toBe("catch_up");
    expect((wrapper.vm as any).triggerEditor.catch_up_limit).toBe(5);
  });
  ...
  it("updates scheduled Trigger through the real PATCH contract and refreshes details", async () => {
    ...
    await (wrapper.vm as any).load();
    await (wrapper.vm as any).openTriggerEditor(scheduledTrigger);
    (wrapper.vm as any).triggerEditor.interval_seconds = 900;
    vi.mocked(workflowApi.triggers).mockClear();
    await (wrapper.vm as any).saveTriggerEditor();
    expect(workflowApi.updateTrigger).toHaveBeenCalledWith("w1", "t-scheduled", { name: "Daily", config: { timezone: "Asia/Seoul", interval_seconds: 900, misfire_policy: "catch_up", catch_up_limit: 5 } });
    expect(workflowApi.triggers).toHaveBeenCalledTimes(1);
    expect((wrapper.vm as any).triggerEditor.trigger).toBeNull();
  });
  ...
  it("updates Webhook Trigger without reading or sending the existing Secret", async () => {
    ...
    (wrapper.vm as any).triggerEditor.event_id_field = "event_id_v2";
    await (wrapper.vm as any).saveTriggerEditor();
    expect(workflowApi.updateTrigger).toHaveBeenCalledWith("w1", "t-webhook", { name: "Webhook", config: { auth_mode: "secret", event_id_field: "event_id_v2" } });
  });
  ...
  it("requires explicit delete confirmation and closes the target state on cancel", async () => {
    ...
    expect((wrapper.vm as any).deleteTriggerTarget.id).toBe("t-scheduled");
    await (wrapper.vm as any).cancelDeleteTrigger();
    expect(workflowApi.deleteTrigger).not.toHaveBeenCalled();
    expect((wrapper.vm as any).deleteTriggerTarget).toBeNull();
  });
