...
describe("AuditLog UI-04 states",()=>{
 ...
 it.each([["empty",()=>Promise.resolve({data:{items:[],total:0}}),"暂无符合条件的审计日志"],["error",()=>Promise.reject(new Error("network")),"审计日志加载失败"],["permission",()=>Promise.reject({response:{status:403}}),"无权查看审计日志"]] as const)("renders %s",async(state,responseFactory,title)=>{api.auditLogs.mockReturnValueOnce(responseFactory());const wrapper=mountView();await flushPromises();expect(wrapper.findComponent(StatePanel).props("state")).toBe(state);expect(wrapper.text()).toContain(title)});
});
