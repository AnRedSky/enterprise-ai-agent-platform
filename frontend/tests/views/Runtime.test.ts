...
const { executions, executionEvents, executionTrace } = vi.hoisted(() => ({
  executions: vi.fn(), executionEvents: vi.fn(), executionTrace: vi.fn(),
}));
...
const stubs = {
  ...
  "el-button": { template: "<button><slot/></button>" }, "el-alert": { template: "<div class=\"alert\">Runtime 查询失败，请稍后重试</div>" },
  "el-empty": { template: "<div class=\"empty\">empty</div>" }, "el-table": { template: "<div class=\"table\"><slot/></div>" },
  "el-table-column": { template: "<div/>" }, "el-pagination": { template: "<div/>" }, "el-drawer": { template: "<div><slot/></div>" },
