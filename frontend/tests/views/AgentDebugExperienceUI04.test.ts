...
describe("AgentDebugExperience UI-04", () => {
  beforeEach(() => {
    ...
    api.getPublishedVersion.mockResolvedValue(version);
  });

  it("renders the loading state while the debug context is loading", async () => {
  ...
  it("renders the empty state when no accessible agent exists", async () => {
    api.listAgents.mockResolvedValue([]);
    const wrapper = mount(AgentDebugExperience, { global });
    await vi.waitFor(() => expect(wrapper.text()).toContain("暂无可调试智能体"));
  ...
  it("renders the shared error state and does not expose raw backend errors", async () => {
    ...
    const wrapper = mount(AgentDebugExperience, { global });
    await vi.waitFor(() => expect(wrapper.text()).toContain("调试上下文加载失败"));
