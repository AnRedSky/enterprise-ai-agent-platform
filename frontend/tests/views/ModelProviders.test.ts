...
describe("Model Provider / Profile management UI", () => {
  ...
  it("loads organization scoped providers and profiles", async () => { const wrapper = mount(ModelProviders, { global }); await vi.waitFor(() => expect(api.listModelProviders).toHaveBeenCalledWith("o1")); await vi.waitFor(() => expect(api.listModelProfiles).toHaveBeenCalledWith("p1")); expect((wrapper.vm as any).providers[0].name).toBe("Local Ollama"); });
  it("uses Chinese wording while preserving technical values", async () => { const wrapper = mount(ModelProviders, { global }); await vi.waitFor(() => expect(api.listModelProviders).toHaveBeenCalled()); const text = wrapper.text(); expect(text).toContain("模型提供方与模型配置"); expect(text).toContain("创建模型提供方"); expect(text).toContain("模型配置"); expect(text).toContain("接口地址"); expect(text).toContain("凭据引用"); expect(text).toContain("Dimension"); expect(text).not.toContain("Model Provider / Profile"); expect(text).not.toContain("Create Provider"); expect(text).not.toContain("enabled"); });
  it("keeps chat dimension unset when saving a profile", async () => { api.createModelProfile.mockResolvedValue({ id: "m2" }); const wrapper = mount(ModelProviders, { global }); await vi.waitFor(() => expect(api.listModelProviders).toHaveBeenCalled()); (wrapper.vm as any).selectedProviderId = "p1"; (wrapper.vm as any).profileForm = { name: "Chat", model_type: "chat", model_name: "chat-model", dimension: 1024, capabilities: {}, parameters: {}, enabled: true, is_default: false }; await (wrapper.vm as any).saveProfile(); expect(api.createModelProfile).toHaveBeenCalledWith("p1", expect.objectContaining({ model_type: "chat", dimension: null })); });
});
