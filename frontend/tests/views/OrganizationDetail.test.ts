...
describe("OrganizationDetail management UI", () => {
  ...
  it("loads organization and membership state", async () => {
    ...
    expect((wrapper.vm as any).totalMembers).toBe(3);
    expect(api.listMembers).toHaveBeenCalledWith("o1", 0, 20);
  ...
  it("does not expose raw backend error text", async () => {
    ...
    const wrapper = mount(OrganizationDetail, { global });
    await vi.waitFor(() => expect((wrapper.vm as any).error).toBe("组织详情加载失败，请稍后重试"));
    expect(wrapper.text()).toContain("组织详情加载失败，请稍后重试");
    expect(wrapper.text()).not.toContain("500 Internal Server Error");
  });

  it("loads a requested membership page using the bounded list contract", async () => {
    const wrapper = mount(OrganizationDetail, { global });
    await vi.waitFor(() => expect((wrapper.vm as any).loading).toBe(false));
    await (wrapper.vm as any).changePage(2);
    expect(api.listMembers).toHaveBeenLastCalledWith("o1", 20, 20);
