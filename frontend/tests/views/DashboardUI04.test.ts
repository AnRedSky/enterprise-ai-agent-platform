...
function mountView() { return mount(DashboardOverview, { global: { stubs: { PageHeader: true, MetricCard: true, SurfaceCard: true, "el-button": true, "el-table": true, "el-table-column": true, "el-tag": true, "el-empty": true, "el-alert": true } } }); }
...
describe("Dashboard UI-04 states", () => {
  ...
  it("renders loading while aggregate APIs are pending", () => {
    mocks.listAgents.mockReturnValue(new Promise(() => {})); mocks.listTools.mockReturnValue(new Promise(() => {})); mocks.executions.mockReturnValue(new Promise(() => {}));
