  ...
  <div class="page">
    ...
    <div class="cards" v-loading="loading">
      ...
      <el-card>
        ...
        <b>{{ metrics.publishedAgents }}</b>
        <span>个可运行 Agent</span>
      </el-card>
      <el-card>
    ...
    <div class="actions">
      <el-button type="primary" @click="$router.push('/agents')">进入 Agent 管理</el-button>
...
.cards { display: grid; grid-template-columns: repeat(5, minmax(140px, 1fr)); gap: 16px; margin: 28px 0; }
.cards :deep(.el-card__header) { font-weight: 600; }
.cards b { display: block; font-size: 30px; line-height: 1.3; }
.cards span { color: #667085; font-size: 13px; }
.actions { display: flex; gap: 12px; }
@media (max-width: 1100px) { .cards { grid-template-columns: repeat(3, minmax(140px, 1fr)); } }
@media (max-width: 700px) { .cards { grid-template-columns: repeat(2, minmax(140px, 1fr)); } }
</style>
