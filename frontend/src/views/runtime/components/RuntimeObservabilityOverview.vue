...
<template>
  <section class="observability-panel" aria-label="运行可观测性概览">
    <div class="panel-heading">
      <div><span class="eyebrow">P1 可观测性</span><h2>运行健康概览</h2><p>先判断运行健康，再进入 Execution、时间线、Trace 与 Audit 诊断。</p></div>
      <div class="heading-actions"><el-tag :type="counts.failed ? 'danger' : 'success'" effect="plain">{{ healthLabel }}</el-tag><el-button size="small" @click="loadOverview" :loading="loading">刷新概览</el-button></div>
    </div>
    ...
    <template v-else>
      ...
      <div class="diagnostic-flow"><span>Execution</span><i>→</i><span>时间线</span><i>→</i><span>Trace</span><i>→</i><span>Audit</span><el-button link type="primary" @click="openRuntime()">进入运行中心</el-button></div>
    </template>
  </section>
</template>
