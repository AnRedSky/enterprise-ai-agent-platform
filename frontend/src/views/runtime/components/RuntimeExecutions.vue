<template>
  <el-card>
    ...
    <el-table v-else :data="items" v-loading="loading" @row-click="open">
      <el-table-column prop="execution_id" label="运行记录 ID" min-width="260">
      ...
      <el-table-column label="状态" width="120">
        <template #default="{ row }"><el-tag :type="getRuntimeStatusMeta(row.status).type">{{ getRuntimeStatusMeta(row.status).label }}</el-tag></template>
      </el-table-column>
      <el-table-column prop="agent_id" label="智能体" min-width="220" />
      <el-table-column label="链路 ID" min-width="220">
        <template #default="{ row }"><el-button link type="primary" @click.stop="copyRuntimeId(row.trace_id)">{{ shortRuntimeId(row.trace_id) }}</el-button></template>
      </el-table-column>
...
async function load() { loading.value = true; error.value = false; try { const r = await runtimeApi.executions({ page: page.value, page_size: pageSize.value, ...(status.value ? { status: status.value } : {}) }); items.value = r.data.items; total.value = r.data.total; } catch { error.value = true; ElMessage.error("运行记录查询失败"); } finally { loading.value = false; } }
async function open(row: Execution) { selected.value = row; events.value = []; traceItems.value = []; detailError.value = false; drawer.value = true; try { const [timeline, trace] = await Promise.all([runtimeApi.executionEvents(row.execution_id), runtimeApi.executionTrace(row.execution_id)]); events.value = timeline.data.items; traceItems.value = trace.data.items; } catch { detailError.value = true; ElMessage.error("运行记录详情查询失败"); } }
async function copyRuntimeId(value: string | null | undefined) { if (!value) return; try { await navigator.clipboard.writeText(value); ElMessage.success("执行上下文已复制"); } catch { ElMessage.error("复制失败，请手动复制"); } }
onMounted(load);
</script>
