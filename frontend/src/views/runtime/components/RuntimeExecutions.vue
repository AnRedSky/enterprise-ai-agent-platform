<template>
  <el-card>
    ...
    <el-table v-else :data="items" v-loading="loading" @row-click="open">
      ...
      <el-table-column prop="trace_id" label="Trace" min-width="220" />
  ...
  <el-drawer v-model="drawer" title="Execution Observability" size="60%">
    ...
    <template v-else>
      <el-descriptions v-if="selected" :column="2" border>
        <el-descriptions-item label="Execution">{{ selected.execution_id }}</el-descriptions-item>
        <el-descriptions-item label="Status">{{ selected.status }}</el-descriptions-item>
        <el-descriptions-item label="Trace">{{ selected.trace_id }}</el-descriptions-item>
        <el-descriptions-item label="Request">{{ selected.request_id }}</el-descriptions-item>
      </el-descriptions>
      ...
      <el-timeline v-else>
        <el-timeline-item v-for="item in traceItems" :key="item.id" :timestamp="item.created_at">
          ...
          <el-descriptions :column="2" border style="margin-top: 8px">
            <el-descriptions-item label="Trace ID">{{ item.trace_id }}</el-descriptions-item>
