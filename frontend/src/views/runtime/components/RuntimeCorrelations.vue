...
<template>
  <section class="correlation-workspace" aria-label="Audit Trace 关联工作台">
    ...
    <template v-if="result">
      ...
      <el-card v-if="focusedAudit" shadow="never" class="fact-card"><template #header><div class="card-title"><span>Audit 具体事实</span><el-button link type="primary" @click="copy(focusedAudit.id)">复制 Audit ID</el-button></div></template><el-descriptions :column="2" border><el-descriptions-item label="Audit ID">{{ focusedAudit.id }}</el-descriptions-item><el-descriptions-item label="操作">{{ focusedAudit.action }}</el-descriptions-item><el-descriptions-item label="Execution ID">{{ focusedAudit.workflow_execution_id || "-" }}</el-descriptions-item><el-descriptions-item label="Workflow ID">{{ focusedAudit.workflow_id || "-" }}</el-descriptions-item><el-descriptions-item label="Workflow Version">{{ focusedAudit.workflow_version_id || "-" }}</el-descriptions-item><el-descriptions-item label="Trace ID">{{ focusedAudit.trace_id || "-" }}</el-descriptions-item><el-descriptions-item label="Actor ID">{{ focusedAudit.actor_id || "-" }}</el-descriptions-item><el-descriptions-item label="Resource">{{ focusedAudit.resource_type }} / {{ focusedAudit.resource_id || "-" }}</el-descriptions-item><el-descriptions-item label="Request ID">{{ focusedAudit.request_id || "-" }}</el-descriptions-item><el-descriptions-item label="状态">{{ focusedAudit.status }}</el-descriptions-item><el-descriptions-item label="错误代码">{{ focusedAudit.error_code || "-" }}</el-descriptions-item><el-descriptions-item label="Metadata" :span="2"><pre class="fact-json">{{ focusedAudit.metadata ? JSON.stringify(focusedAudit.metadata, null, 2) : "-" }}</pre></el-descriptions-item></el-descriptions></el-card>
