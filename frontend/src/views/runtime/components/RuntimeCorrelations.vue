...
<template>
  <section class="correlation-workspace" aria-label="Audit Trace 关联工作台">
    ...
    <template v-if="result">
      <el-card shadow="never" class="relation-card"><template #header><div class="card-title"><span>Execution</span><el-tag v-if="result.execution" :type="result.execution.status === 'failed' ? 'danger' : 'success'">{{ result.execution.status }}</el-tag></div></template><el-empty v-if="!result.execution" description="没有可访问的 Execution" :image-size="60" /><el-descriptions v-else :column="2" border><el-descriptions-item label="Execution ID"><el-button link type="primary" @click="copy(result.execution.id)">{{ shortRuntimeId(result.execution.id) }}</el-button></el-descriptions-item><el-descriptions-item label="Workflow ID">{{ shortRuntimeId(result.execution.workflow_id) }}</el-descriptions-item><el-descriptions-item label="Workflow Version">{{ shortRuntimeId(result.execution.workflow_version_id) }}</el-descriptions-item><el-descriptions-item label="状态">{{ result.execution.status }}</el-descriptions-item><el-descriptions-item label="错误代码">{{ result.execution.error_code || "-" }}</el-descriptions-item><el-descriptions-item label="创建时间">{{ result.execution.created_at }}</el-descriptions-item></el-descriptions></el-card>
      <el-card shadow="never" class="relation-card"><template #header><div class="card-title"><span>Trace</span><el-tag>{{ result.traces.total }}</el-tag></div></template><el-table :data="result.traces.items" size="small"><el-table-column prop="event_type" label="事件" min-width="220" /><el-table-column prop="status" label="状态" width="110" /><el-table-column label="Trace ID" min-width="200"><template #default="{ row }"><el-button link type="primary" @click="copy(row.trace_id)">{{ shortRuntimeId(row.trace_id) }}</el-button></template></el-table-column><el-table-column prop="created_at" label="时间" min-width="180" /></el-table><el-pagination v-if="result.traces.total > result.traces.page_size" :current-page="result.traces.page" :page-size="result.traces.page_size" :total="result.traces.total" layout="prev, pager, next" @current-change="setTracePage" /></el-card>
