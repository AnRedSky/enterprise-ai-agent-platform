...
function requestInvoke(trigger: WorkflowTrigger) {
  invokeTarget.value = trigger;
...
async function confirmInvoke() {
  ...
  try {
    ...
    if (e?.response?.status === 403) ElMessage.error("当前账号没有执行该触发器的权限。");
    else ElMessage.error("手动触发失败，请检查工作流发布状态后重试。");
  } finally {
