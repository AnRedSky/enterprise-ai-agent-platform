...
async function load() {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : "Organization 列表加载失败";
  } finally {
    loading.value = false;
  }
}
...
async function create() {
  ...
  try {
    ...
    await load();
    ElMessage.success("Organization 创建成功");
  } catch (e) {
    ElMessage.error(e instanceof Error ? e.message : "Organization 创建失败");
  } finally {
    saving.value = false;
  }
}
...
<template>
  <div class="page">
    <div class="header">
      <div><h1>Organizations</h1><p>企业组织与成员访问治理。</p></div>
      <el-button type="primary" @click="dialogVisible = true">创建 Organization</el-button>
    </div>
    ...
    <el-empty v-if="!loading && !organizations.length" description="暂无 Organization。" />
    <el-dialog v-model="dialogVisible" title="创建 Organization" width="480px">
