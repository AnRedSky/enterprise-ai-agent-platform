...
const organization = ref<Organization | null>(null); const members = ref<Membership[]>([]); const currentMembership = ref<Membership | null>(null); const loading = ref(true); const membersLoading = ref(false); const error = ref(""); const memberError = ref(""); const saving = ref(false);
...
async function loadCurrentMembership() { const userId = getUserId(); if (!userId) { currentMembership.value = null; return; } try { const limit = 100; for (let offset = 0;; offset += limit) { const response = await listMembers(id, offset, limit); const membership = response.items.find((item) => item.user_id === userId); if (membership) { currentMembership.value = membership; return; } if (offset + response.items.length >= response.total) break; } currentMembership.value = null; } catch { currentMembership.value = null; } }
async function load() { loading.value = true; error.value = ""; try { organization.value = await getOrganization(id); await Promise.all([loadMembers(1), loadCurrentMembership()]); } catch (e) { organization.value = null; currentMembership.value = null; error.value = organizationLoadError(e); } finally { loading.value = false; } }
