...
async function load() { loading.value = true; error.value = ""; try { providers.value = (await listModelProviders(organizationId)).items; await Promise.all(providers.value.map((provider) => loadProfiles(provider.id))); } catch (e) { console.error("模型提供方列表加载失败", e); error.value = userError(e, "模型提供方列表加载失败，请稍后重试"); } finally { loading.value = false; } }
...
async function saveProvider() { if (!providerForm.value.name.trim() || !providerForm.value.provider_name.trim()) return; saving.value = true; try { if (editingProvider.value) await updateModelProvider(editingProvider.value.id, providerForm.value as ModelProviderUpdatePayload); else await createModelProvider(providerForm.value); providerDialog.value = false; await load(); ElMessage.success("模型提供方保存成功"); } catch (e) { console.error("模型提供方保存失败", e); ElMessage.error(userError(e, "模型提供方保存失败，请稍后重试")); } finally { saving.value = false; } }
async function removeProvider(provider: ModelProvider) { try { await ElMessageBox.confirm("删除模型提供方前，请先确认没有模型配置正在使用它。", "删除模型提供方", { type: "warning" }); await deleteModelProvider(provider.id); await load(); ElMessage.success("模型提供方已删除"); } catch (e) { if (e !== "cancel" && e !== "close") { console.error("模型提供方删除失败", e); ElMessage.error(userError(e, "模型提供方删除失败，请稍后重试")); } } }
...
function editProfileFromTableRow(row: unknown) { openEditProfile(row as ModelProfile); }
async function saveProfile() { if (!profileForm.value.name.trim() || !profileForm.value.model_name.trim() || !selectedProviderId.value) return; const payload = { ...profileForm.value, dimension: profileForm.value.model_type === "embedding" ? profileForm.value.dimension : null }; saving.value = true; try { if (editingProfile.value) await updateModelProfile(editingProfile.value.id, payload); else await createModelProfile(selectedProviderId.value, payload); profileDialog.value = false; await loadProfiles(selectedProviderId.value); ElMessage.success("模型配置保存成功"); } catch (e) { console.error("模型配置保存失败", e); ElMessage.error(userError(e, "模型配置保存失败，请稍后重试")); } finally { saving.value = false; } }
async function removeProfile(profile: ModelProfile) { try { await ElMessageBox.confirm("删除模型配置后，不能再用于运行记录或评估。", "删除模型配置", { type: "warning" }); await deleteModelProfile(profile.id); await loadProfiles(profile.provider_id); ElMessage.success("模型配置已删除"); } catch (e) { if (e !== "cancel" && e !== "close") { console.error("模型配置删除失败", e); ElMessage.error(userError(e, "模型配置删除失败，请稍后重试")); } } }
function removeProfileFromTableRow(row: unknown) { void removeProfile(row as ModelProfile); }
onMounted(load);
</script>
<template>
  <div class="page">
