...
async function openChunks(version: unknown) {
  ...
  try {
    ...
    ElMessage.error(e instanceof Error ? e.message : "Chunk 加载失败");
  }
}
...
<template>
  <div class="page">
    ...
    <el-card v-if="selectedVersion" class="section">
      <template #header><b>Chunk：{{ selectedVersion.version }}</b></template>
      <el-table :data="chunks">
        <el-table-column prop="chunk_index" label="#" width="60" />
        <el-table-column prop="start_offset" label="Start" width="80" />
        <el-table-column prop="end_offset" label="End" width="80" />
