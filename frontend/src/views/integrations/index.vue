...
async function createDestination() {
  ...
  } catch (error) { ElMessage.error(error instanceof Error ? error.message : "投递目标创建失败"); }
}
async function createSubscription() {
  ...
  try {
    ...
    subscriptionDialog.value = false; subscriptionForm.event_type = ""; await loadData(); ElMessage.success("事件订阅已创建");
  } catch (error) { ElMessage.error(error instanceof Error ? error.message : "事件订阅创建失败"); }
}
...
<template>
  <div class="integration-page">
    <div class="page-heading">
      <div class="heading-copy"><div class="eyebrow">平台 / 系统集成</div><h1>集成中心</h1><p>将平台产生的可靠事件安全、稳定地发送到企业内部系统和业务服务。</p></div>
      <el-button :icon="Refresh" :loading="loading" @click="loadData">刷新数据</el-button>
    </div>

    <div class="metric-grid">
      <div class="metric-card"><div class="metric-icon"><Link /></div><div><span>投递目标</span><strong>{{ destinations.length }}</strong><small><CircleCheck /> {{ enabledDestinations }} 个已启用</small></div></div>
      ...
      <div class="metric-card"><div class="metric-icon"><Connection /></div><div><span>事件类型</span><strong>{{ eventTypeCount }}</strong><small>当前订阅覆盖的不同事件类型</small></div></div>
    </div>
    ...
    <el-card class="workspace-card" shadow="never">
      <el-tabs v-model="activeTab">
        <el-tab-pane name="destinations">
          <template #label><span class="tab-label"><Link /> 投递目标</span></template>
          ...
          <el-table v-loading="loading" :data="destinations" empty-text="暂无投递目标">
            <el-table-column prop="name" label="名称" min-width="180" />
            <el-table-column prop="endpoint_url" label="目标地址" min-width="300" show-overflow-tooltip />
        ...
        <el-tab-pane name="subscriptions">
          ...
          <div class="toolbar"><div><strong>事件订阅</strong><span>按照事件类型，将平台产生的事件发送到指定投递目标。</span></div><el-button type="primary" :icon="Plus" :disabled="!destinations.length" @click="subscriptionDialog = true">新建事件订阅</el-button></div>
          <el-table v-loading="loading" :data="subscriptions" empty-text="暂无事件订阅">
            <el-table-column prop="event_type" label="事件类型" min-width="240" />
            <el-table-column label="投递目标" min-width="180"><template #default="scope">{{ destinationName(scope.row.destination_id) }}</template></el-table-column>
        ...
        <el-tab-pane name="deliveries">
          <template #label><span class="tab-label"><Connection /> 投递管理</span></template>
          <DeliveryConsole />
        </el-tab-pane>
      </el-tabs>
    </el-card>

    <el-dialog v-model="destinationDialog" title="新建投递目标" width="560px" @closed="resetDestinationForm">
    ...
    <el-dialog v-model="subscriptionDialog" title="新建事件订阅" width="520px">
