<template>
  <div class="operations-console">
    <div class="toolbar">
      <div><strong>Runtime 企业运维中心</strong><span>统一观察 Event、Delivery、SLO、Provider、Alert、Metrics、Audit 与死信；所有数据严格限制在当前租户。</span></div>
      <div class="toolbar-actions"><el-select v-model="windowHours" style="width: 120px" @change="loadAll"><el-option label="最近 1 小时" :value="1" /><el-option label="最近 24 小时" :value="24" /><el-option label="最近 7 天" :value="168" /></el-select><el-button :icon="Refresh" :loading="loading" @click="loadAll">刷新</el-button></div>
    </div>
    <el-alert v-if="error" type="error" :closable="false" title="Runtime 运维数据加载失败，请检查 Runtime API 与数据库连接。" show-icon />
...
import { runtimeOperationsApi, type RuntimeAlert, type RuntimeAlertRule, type RuntimeDeadLetter, type RuntimeMetricSample, type RuntimeOperationsOverview, type RuntimeProvider } from "@/api/runtimeOperations";
...
const loading = ref(false); const error = ref(false); const loadingAlerts = ref(false); const loadingProviders = ref(false); const auditLoading = ref(false); const metricLoading = ref(false); const deadLetterLoading = ref(false); const actionLoading = ref(false);
const deadLetterPage = ref(1); const deadLetterPageSize = ref(20); const deadLetterTotal = ref(0); const replayingId = ref(""); const togglingProviderId = ref(""); const probingProviderId = ref(""); const togglingRuleId = ref("");
const alertEvaluationMessage = ref(""); const providerMessage = ref(""); const metricName = ref("runtime.notification.delivery"); const metricWindow = ref(60); const dimensionKey = ref(""); const dimensionValue = ref("");
...
async function loadAll() { loading.value = true; error.value = false; try { await Promise.all([loadOverview(), loadAlerts(), loadProviders(), loadAudit(), loadDeadLetters()]); } catch { error.value = true; } finally { loading.value = false; } }
...
async function probeProvider(row: RuntimeProvider) { probingProviderId.value = row.id; try { const result = await runtimeOperationsApi.probeProviderHealth(row.id); row.status = result.data.status; row.last_checked_at = new Date().toISOString(); providerMessage.value = result.data.error ? `Provider ${row.name} 探测失败：${result.data.error}` : `Provider ${row.name} 探测完成：${result.data.status}${result.data.latency_ms == null ? "" : `，${result.data.latency_ms} ms`}`; } catch (error) { ElMessage.error(messageFromError(error, "Provider 健康探测失败")); } finally { probingProviderId.value = ""; } }
...
<style scoped>
.operations-console{padding-top:4px}.toolbar{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-bottom:12px}.toolbar>div:first-child,.tab-toolbar>div:first-child{display:grid;gap:4px}.toolbar strong,.tab-toolbar strong{color:#344054;font-size:14px}.toolbar span,.tab-toolbar span{color:#98a2b3;font-size:12px}.toolbar-actions{display:flex;align-items:center;gap:8px}.metric-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:14px}.metric-card{padding:15px 16px;border:1px solid #eaecf0;border-radius:10px;background:#fff}.metric-card span,.metric-card small{display:block;color:#98a2b3;font-size:11px}.metric-card strong{display:block;margin:5px 0;color:#101828;font-size:24px}.metric-card.danger strong{color:#d92d20}.slo-grid{display:grid;grid-template-columns:1.3fr 1fr 1fr;gap:12px}.card-title,.tab-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px}.slo-row,.status-row{display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #f2f4f7;color:#667085;font-size:12px}.slo-row strong,.status-row strong{color:#344054}.slo-row:last-of-type,.status-row:last-child{border-bottom:0}.inline-alert{margin:10px 0}.operations-tabs{background:#fff;border-radius:10px}.operations-tabs :deep(.el-tabs__content){padding:0 2px 12px}.tab-toolbar{margin:8px 0 14px}.el-form{margin-bottom:14px}.el-divider{margin:20px 0}@media(max-width:1000px){.metric-grid{grid-template-columns:repeat(2,1fr)}.slo-grid{grid-template-columns:1fr}}@media(max-width:680px){.metric-grid{grid-template-columns:1fr}.toolbar,.tab-toolbar{align-items:stretch;flex-direction:column}.toolbar-actions{width:100%}.toolbar-actions .el-select{flex:1}.el-form{display:grid}.el-form .el-input,.el-form .el-input-number{width:100%!important}}
</style>
