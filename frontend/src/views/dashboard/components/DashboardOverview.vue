<template>
  <div class="page">
    ...
    <template v-if="pageState === 'success' || pageState === 'empty'">
      <section class="metrics" v-loading="loading" aria-label="平台核心指标"><MetricCard v-for="metric in metricCards" :key="metric.key" :data-testid="`metric-${metric.key}`" :label="metric.label" :value="metric.value" :description="metric.description" :trend="metric.caption" trend-direction="neutral" /></section>
