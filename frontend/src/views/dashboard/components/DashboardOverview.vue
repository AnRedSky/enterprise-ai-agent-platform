<template>
  <div class="page">
    <PageHeader
      eyebrow="企业级智能体平台"
      title="平台工作台"
      description="统一查看智能体、工具和运行记录，快速进入治理与故障处理入口。"
    >
      <template #actions>
    ...
    <section class="workspace-grid">
      <SurfaceCard title="最近执行" description="最近 8 条运行记录">
        <template #header>
      ...
      <SurfaceCard title="常用入口" description="按业务职责快速进入">
...
const loading = ref(false), error = ref(""), recentExecutions = ref<Execution[]>([]);
...
.error { margin-bottom: var(--ui-space-5); }
