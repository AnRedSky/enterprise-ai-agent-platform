<template>
  <div class="page">
    ...
    <template v-else>
      ...
      <section v-if="metrics.failedExecutions > 0" class="attention"><el-alert title="存在失败的运行记录" :description="`当前共有 ${metrics.failedExecutions} 次失败运行记录，建议进入运行记录查看执行链路与错误信息。`" type="warning" show-icon :closable="false"><template #default><el-button type="warning" link @click="$router.push('/runtime')">立即处理</el-button></template></el-alert></section>
    </template>
  </div>
</template>
