...
企业级 AI Agent 平台。当前统一在 `main` 分支推进。Phase 1.x、Phase 2.1、Phase 2.2、Phase 2.3、Phase 2.5、Phase 2.6 已完成主线收口；**Phase 2.7 Advanced Workflow Orchestration 主线生产开发已完成，本地 Backend Unit / Default Regression 已实际通过，当前进入 Migration / Real API / Frontend / E2E 验收收口。**
...
- [Backend 模块架构](docs/00-architecture/BACKEND_MODULE_ARCHITECTURE.md)
...
- [Phase 2.8-A Multi-Agent Collaboration Contract](docs/02-phases/PHASE_2_8_A_CONTRACT.md)
- [错误记录](docs/04-errors/)

## 当前开发状态

实时任务进度、阻塞项和实际测试结果统一维护在 `docs/PROJECT_STATUS.md`；长期工程规则统一维护在 `docs/01-governance/DEVELOPMENT.md`。
...
2.7 主线生产实现                 ← 已完成
        ↓
Backend Unit / Default Regression ← 已通过
        ↓
Migration / DB verification       ← 当前
        ↓
Real HTTP API Gate
        ↓
Frontend Gate
...
```text
Phase 2.8-A Multi-Agent Contract   ← 已启动
        ↓
...
Phase 2.8-A 首版只解决受治理的 Agent Delegation：tenant / version / permission guard、幂等、上下文隔离、depth / concurrency / timeout / model budget、Worker fencing、Audit / Trace 父子链路；不引入第二套 Workflow Retry / Recovery 状态机。
...
后端配置模板：`backend/.env.example`；前端配置模板：`frontend/.env.example`。真实 Secret 不得提交到 Git。
...
```text
Phase 2.7 affected targeted: 27 passed
Durable Resume / Execution / DAG / Frontier targeted regression: 96 passed
Backend full unit regression + RuntimeWarning gate: 811 passed, 3 skipped, 41 deselected
Backend default regression: 811 passed, 3 skipped, 41 deselected
```

完整 Unit Regression 已实际通过且无 RuntimeWarning。当前尚未执行并因此不标记 PASS 的 Gate：Migration、Real HTTP API、Frontend、Browser/E2E、最终人工场景验收。
